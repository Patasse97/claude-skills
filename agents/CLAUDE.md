...
- **cs- prefix for slug conflicts only** â€” applies only on the ClawHub registry when another publisher already owns the slug. Repo folder names and local skill names are never renamed.
- **No paid/commercial service dependencies** â€” skills must not require paid third-party API keys or commercial services unless provided by the project itself.
- **plugin.json** â€” ONLY fields: `name`, `description`, `version`, `author`, `homepage`, `repository`, `license`, `skills: "./"`.
...
âŒ Hardcoding absolute paths
âŒ Skipping YAML frontmatter validation
âŒ Forgetting to test relative paths
âŒ Documenting workflows without examples
âŒ Creating agent dependencies (keep them independent)
âŒ Duplicating skill content in agent files
âŒ Using LLM calls instead of referencing Python tools
