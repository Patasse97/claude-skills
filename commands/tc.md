...
2. Determine the user's intent from natural language:
   - **Status change** â†’ `--set-status <state>` with `--reason "<why>"`
   - **Add files** â†’ one or more `--add-file path[:action]`
   - **Add a test** â†’ `--add-test "<title>" --test-procedure "<step>" --test-expected "<result>"`
   - **Update handoff** â†’ any combination of `--handoff-progress`, `--handoff-next`, `--handoff-blocker`, `--handoff-context`
   - **Add a note** â†’ `--note "<text>"`
   - **Add a tag** â†’ `--tag <tag>`
3. Run:
   ```bash
   python3 engineering/tc-tracker/scripts/tc_update.py --root . --tc-id <tc-id> [flags] --json
   ```
4. If exit code is non-zero, surface the error verbatim. The state machine and validator will reject invalid moves â€” do not retry blindly.
...
4. If yes, run an update to record the resumption:
   ...
   ```
5. Begin executing the first item in `next_steps`. Do NOT re-derive context â€” trust the handoff.
...
5. Run:
   ```bash
   python3 engineering/tc-tracker/scripts/tc_update.py --root . --tc-id <tc-id> \
     --set-status deployed --reason "Approved by <approver>" --note "Approval: <approver> â€” <notes>"
   ```
...
3. **Never delete a TC.** History is append-only â€” add a final revision and tag it `[CANCELLED]`.
...
- `engineering/tc-tracker` â€” Full SKILL.md with schema reference, lifecycle diagrams, and the handoff format.
- `engineering/changelog-generator` â€” Pair with TC tracker: TCs for the per-change audit trail, changelog for user-facing release notes.
- `engineering/tech-debt-tracker` â€” For tracking long-lived debt rather than discrete code changes.
