---
title: "/plugin-audit â€” Slash Command for AI Coding Agents"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/commands/plugin-audit.md">Source</a></span>
</div>
...
2. Read `SKILL.md` frontmatter â€” extract `name`, `description`, `Category`, `Tier`
3. Detect skill type:
   - Has `scripts/` â†’ has Python tools
   - Has `references/` â†’ has reference docs
   - Has `assets/` â†’ has templates/samples
   - Has `expected_outputs/` â†’ has test fixtures
   - Has `agents/` â†’ has embedded agents
   - Has `skills/` â†’ has sub-skills (compound skill)
   - Has `.claude-plugin/plugin.json` â†’ is a standalone plugin
   - Has `settings.json` â†’ has command registrations
...
**Gate rule:** Score must be â‰¥ 75 (GOOD). If below 75:
- Read the errors list
- Auto-fix what's possible:
  - Missing frontmatter fields â†’ add them from SKILL.md content
  - Missing sections â†’ add stub headings
  - Missing directories â†’ create empty ones with a note
...
**Gate rule:** Score must be â‰¥ 60 (C). If below 60, report the improvement roadmap items as action items.
...
**Auto-fix:** If a script fails the `--help` test, check if it has `argparse` â€” if not, this is a real issue. If it fails the stdlib-only test, flag the import and **ask the user** whether the dependency is acceptable (this is a critical decision).
...
**Gate rule:** Zero CRITICAL findings. Zero HIGH findings. Any CRITICAL or HIGH is a blocker â€” report the exact file, line, pattern, and recommended fix.
...
1. Parse as JSON â€” must be valid
...
**How to invoke:** Read the agent's `.md` file to understand its review criteria. Apply those criteria to review the skill's SKILL.md, scripts, and references. This is NOT spawning a subagent â€” it's using the agent's documented perspective to structure your review.
...
â•‘  PLUGIN AUDIT REPORT: {skill_name}                         â•‘
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
â•‘                                                              â•‘
â•‘  Phase 1 â€” Discovery          âœ… {type}, {domain}            â•‘
â•‘  Phase 2 â€” Structure          âœ… {score}/100 ({level})       â•‘
â•‘  Phase 3 â€” Quality            âœ… {score}/100 ({grade})       â•‘
â•‘  Phase 4 â€” Scripts            âœ… {n}/{n} PASS                â•‘
â•‘  Phase 5 â€” Security           âœ… PASS (0 critical, 0 high)   â•‘
â•‘  Phase 6 â€” Marketplace        âœ… plugin.json valid            â•‘
â•‘  Phase 7 â€” Ecosystem          âœ… Codex + Gemini synced        â•‘
â•‘  Phase 8 â€” Code Review        âœ… {domain} review passed       â•‘
â•‘                                                              â•‘
â•‘  VERDICT: âœ… PASS â€” Ready for merge/publish                  â•‘
â•‘                                                              â•‘
â•‘  Auto-fixes applied: {n}                                     â•‘
â•‘  Warnings: {n}                                               â•‘
â•‘  Action items: {n}                                           â•‘
â•‘                                                              â•‘
...
| Condition | Verdict |
|-----------|---------|
| All phases pass | **PASS** â€” Ready for merge/publish |
| Only warnings (no blockers) | **PASS WITH WARNINGS** â€” Review warnings before merge |
| Any phase has a blocker | **FAIL** â€” List blockers with fix instructions |
