...
All 156 skills in this repository can be converted to work natively with **7 AI coding tools** beyond Claude Code, Codex, and Gemini CLI. The conversion preserves skill instructions, workflows, and supporting files â€” adapting only the format each tool expects.
...
Tools that use flat files (Cursor, Aider, Kilo Code, Augment) get the SKILL.md body only â€” supporting directories are not copied since those tools don't support subdirectories per rule.
...
- **`alwaysApply: false`** â€” skills are available on-demand, not always loaded
- **`globs:`** â€” empty by default; add file patterns to auto-activate for specific files (e.g., `*.test.ts`)
...
```markdown
# Claude Skills â€” Aider Conventions
...
Open Kilo Code's rules panel (click the âš– icon) to see all rules loaded.
...
[Windsurf](https://windsurf.com) uses the same `SKILL.md` format as Claude Code â€” skills convert with minimal changes.
...
â”œâ”€â”€ SKILL.md           # Instructions with name/description frontmatter
â”œâ”€â”€ scripts/           # Python tools (if present in source)
â”œâ”€â”€ references/        # Domain knowledge (if present)
â””â”€â”€ templates/         # Code templates (if present)
```
...
Windsurf uses progressive disclosure â€” only the skill name and description are shown by default. The full `SKILL.md` content loads only when Windsurf decides the skill is relevant to your request, keeping your context window lean.
...
OpenCode also reads `.claude/skills/` directories. If you already have skills installed for Claude Code, OpenCode will discover them automatically â€” no conversion needed.
...
- **`type: auto`** â€” Augment automatically activates the rule when it matches your request
...
- **`risk: low`** â€” all skills are instruction-only, no dangerous operations
- **`source: community`** â€” identifies these as community-contributed skills
- **`date_added`** â€” conversion date for tracking freshness
...
??? question "Some skills show garbled descriptions"
    This can happen with skills using complex YAML multi-line descriptions. Re-run `convert.sh` â€” the parser handles folded (`>`) and literal (`|`) YAML scalars.

??? question "Can I use skills from multiple tools at once?"
    Yes! You can install skills for Cursor and Windsurf in the same project â€” they use different directories and won't conflict.
