---
title: "Agent Plugin Marketplace â€” Codex & OpenClaw Plugins"
...
**28 installable plugins** â€” domain bundles and standalone packages distributed via Claude Code plugin registry and ClawHub.
...
=== "OpenClaw"
    ...
    ```bash
    curl -sL https://raw.githubusercontent.com/Patasse97/claude-skills/main/scripts/openclaw-install.sh | bash
    ```
...
### :material-code-braces: Engineering â€” Core
...
### :material-rocket-launch: Engineering â€” POWERFUL
...
!!! info "ClawHub Registry"
    Plugins are distributed via [ClawHub](https://clawhub.com) as the public registry. The `cs-` prefix is used only when a slug is already taken by another publisher â€” repo folder names remain unchanged.
...
â”‚   â”œâ”€â”€ scripts/           # Python CLI tools
â”‚   â”œâ”€â”€ references/        # Knowledge bases
â”‚   â””â”€â”€ assets/            # Templates
â”œâ”€â”€ skill-b/
â”‚   â””â”€â”€ ...
â””â”€â”€ CLAUDE.md              # Domain instructions
```
...
!!! tip "Full feature support"
    Slash commands (like `/pw:generate`) and agent spawning (like `cs-engineering-lead`) are Claude Code features. On other platforms, the underlying skills and Python tools work â€” just without the command/agent wrappers.
...
??? question "What's the difference between a bundle and a standalone plugin?"
    **Bundles** install all skills in a domain (e.g., `marketing-skills` installs all 43 marketing skills). **Standalone** plugins install a single skill or toolkit (e.g., `pw` installs just Playwright Pro). Standalone plugins are subsets of their parent bundle â€” installing both is safe but redundant.
...
??? question "Do plugins require API keys or paid services?"
    No. All Python tools use stdlib only. Some skills reference external services (AWS, Google Workspace, Stripe) but follow a BYOK (bring-your-own-key) pattern â€” you use your own accounts.
...
??? question "Can I use skills without installing anything?"
    Yes. We have [6 Custom GPTs for ChatGPT](../custom-gpts.md) that package Agent Skills into a conversational interface â€” no installation, no API keys. Just click and chat.
