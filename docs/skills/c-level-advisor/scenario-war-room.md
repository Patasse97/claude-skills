---
title: "Scenario War Room â€” Agent Skill for Executives"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/c-level-advisor/scenario-war-room/SKILL.md">Source</a></span>
</div>
...
Model cascading what-if scenarios across all business functions. Not single-assumption stress tests â€” compound adversity that shows how one problem creates the next.
...
- **Not** financial modeling only â€” every function gets modeled
- **Not** worst-case-only â€” models 3 severity levels
- **Not** paralysis by analysis â€” outputs concrete hedges and triggers

## Framework: 6-Step Cascade Model

### Step 1: Define Scenario Variables (max 3)
State each variable with:
- **What changes** â€” specific, quantified if possible
- **Probability** â€” your best estimate
- **Timeline** â€” when it hits
...
TRIGGER: Customer churn ($560K ARR)
  â†“
CFO: Runway drops 14 â†’ 8 months
  â†“
CHRO: Hiring freeze; retention risk increases (morale hit)
  â†“
CTO: 3 open engineering reqs frozen; roadmap slips
  â†“
CPO: Q4 feature launch delayed â†’ customer retention risk
  â†“
CRO: NRR drops; existing accounts see reduced velocity â†’ more churn risk
  â†“
CFO: [Secondary cascade â€” potential death spiral if not interrupted]
```
...
SEVERITY LEVELS
Base (A only): [runway/ARR impact] â€” recovery: [X actions]
Stress (A+B): [runway/ARR impact] â€” recovery: [X actions]
Severe (A+B+C): [runway/ARR impact] â€” existential risk: [yes/no]

CASCADE MAP
[A â†’ domain impact â†’ B trigger â†’ domain impact â†’ end state]

EARLY WARNING SIGNALS
- [Signal 1 â†’ which scenario it indicates]
- [Signal 2 â†’ which scenario it indicates]
- [Signal 3 â†’ which scenario it indicates]

HEDGES (take these actions now)
1. [Action] â€” cost: $X â€” impact: [what it buys] â€” owner: [role] â€” deadline: [date]
2. [Action] â€” cost: $X â€” impact: [what it buys] â€” owner: [role] â€” deadline: [date]
3. [Action] â€” cost: $X â€” impact: [what it buys] â€” owner: [role] â€” deadline: [date]
...
**Max 3 variables per scenario.** More than 3 is noise â€” you can't meaningfully prepare for 5-variable collapse. Model the 3 that actually worry you.
...
## References
- `references/scenario-planning.md` â€” Shell methodology, pre-mortem, Monte Carlo, cascade frameworks
- `scripts/scenario_modeler.py` â€” CLI tool for structured scenario modeling
