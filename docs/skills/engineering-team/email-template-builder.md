---
title: "Email Template Builder â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/email-template-builder/SKILL.md">Source</a></span>
</div>
...
â”œâ”€â”€ components/
â”‚   â”œâ”€â”€ layout/
â”‚   â”‚   â”œâ”€â”€ email-layout.tsx       # Base layout with brand header/footer
...
â”œâ”€â”€ lib/
â”‚   â”œâ”€â”€ send.ts                    # Unified send function
â”‚   â”œâ”€â”€ providers/
â”‚   â”‚   â”œâ”€â”€ resend.ts
â”‚   â”‚   â”œâ”€â”€ postmark.ts
â”‚   â”‚   â””â”€â”€ ses.ts
â”‚   â””â”€â”€ tracking.ts                # UTM + analytics
â”œâ”€â”€ i18n/
â”‚   â”œâ”€â”€ en.ts
â”‚   â””â”€â”€ de.ts
â””â”€â”€ preview/                       # Dev preview server
    â””â”€â”€ server.ts
```
  ...
  return (
    <EmailLayout preview={`Welcome to MyApp, ${name}! Confirm your email to get started.`}>
      ...
      <Text style={styles.text}>
        We're excited to have you on board. You've got {trialDays} days to explore everything MyApp has to offer â€” no credit card required.
      </Text>
      <Text style={styles.text}>
...
export async function sendEmail(to: string, payload: EmailPayload) {
  const templates = {
    welcome: { component: WelcomeEmail, subject: "Welcome to MyApp â€” confirm your email" },
...
export const de = {
  welcome: {
    ...
    cta: "E-Mail-Adresse bestÃ¤tigen",
  },
}
...
- [ ] No URL shorteners â€” use full branded links
- [ ] No red-flag words: "guarantee", "no risk", "limited time offer" in subject
- [ ] Single CTA per email â€” no 5 different buttons
- [ ] Image alt text on every image
- [ ] HTML validates â€” no broken tags
...
- **Inline styles required** â€” most email clients strip `<head>` styles; React Email handles this
- **Max width 600px** â€” anything wider breaks on Gmail mobile
- **No flexbox/grid** â€” use `<Row>` and `<Column>` from react-email, not CSS grid
- **Dark mode media queries** â€” must use `!important` to override inline styles
- **Missing plain text** â€” all major providers have a plain text field; always populate it
- **Transactional vs marketing** â€” use separate sending domains/IPs to protect deliverability
