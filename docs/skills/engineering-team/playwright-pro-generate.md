---
title: "Generate Playwright Tests â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/playwright-pro/skills/generate/SKILL.md">Source</a></span>
</div>
...
Adapt the template to the specific app â€” replace `{{placeholders}}` with actual selectors, URLs, and data.
...
3. `getByText()` â€” non-interactive text content
4. `getByPlaceholder()` â€” inputs with placeholder text
5. `getByTestId()` â€” when semantic options aren't available

**Assertions** â€” always web-first:
```typescript
// GOOD â€” auto-retries
...
- If project uses TypeScript â†’ generate `.spec.ts`
- If project uses JavaScript â†’ generate `.spec.js` with `require()` imports
- If project has page objects â†’ use them instead of inline locators
- If project has custom fixtures â†’ import and use them
- If project has a test data directory â†’ create test data files there
