---
title: "Red Team â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/red-team/SKILL.md">Source</a></span>
</div>
...
This skill provides the methodology and tooling for **red team engagement planning** â€” building structured attack plans from MITRE ATT&CK technique selection, access level, and crown jewel targets. It scores techniques by effort and detection risk, assembles kill-chain phases, identifies choke points, and flags OPSEC risks.

### Distinction from Other Security Skills

| Skill | Focus | Approach |
|-------|-------|----------|
| **red-team** (this) | Adversary simulation | Offensive â€” structured attack planning and execution |
| security-pen-testing | Vulnerability discovery | Offensive â€” systematic exploitation of specific weaknesses |
| threat-detection | Finding attacker activity | Proactive â€” detect TTPs in telemetry |
| incident-response | Active incident management | Reactive â€” contain and investigate confirmed incidents |
...
```bash
# Basic engagement plan â€” external access, specific techniques
...
|-------|------------------|----------------------|
| external | No internal access â€” internet only | External-facing techniques only (T1190, T1566, etc.) |
| internal | Network foothold â€” no credentials | Internal recon + lateral movement prep |
...
| 2 | Scope violation â€” technique outside access-level constraints |
...
Each phase must be completed before advancing to the next unless the engagement scope specifies assumed breach (skip to a later phase). Do not skip persistence before attempting lateral movement â€” persistence ensures operational continuity if a single foothold is detected and removed.
...
```
effort_score = detection_risk Ã— (len(prerequisites) + 1)
```
...
Prioritize detection rule development and monitoring density around choke point techniques â€” hardening a choke point has multiplied defensive value.
...
|-----------------|------------------|--------------|
| Domain Controller | AD DS, NTDS.dit, SYSVOL | Kerberoasting â†’ DCSync â†’ Golden Ticket |
| Database servers | Production SQL, NoSQL, data warehouse | Lateral movement â†’ DBA account â†’ data staging |
| Payment systems | PCI-scoped network, card data vault | Network pivot â†’ service account â†’ exfiltration |
| Source code repositories | Internal Git, build systems | VPN â†’ internal git â†’ code signing keys |
| Cloud management plane | AWS management console, IAM admin | Phishing â†’ credential â†’ AssumeRole chain |

Crown jewel definition is agreed upon in the RoE â€” engagement success is measured by whether red team reaches defined crown jewels, not by the number of vulnerabilities found.
...
```
external
  â””â”€ T1566.001 (spearphishing) â†’ initial_access
       â””â”€ T1059.001 (PowerShell) â†’ execution
            â””â”€ T1003.001 (LSASS dump) â†’ credential_access [CHOKE POINT]
                 â””â”€ T1550.002 (Pass-the-Hash) â†’ lateral_movement
                      â””â”€ T1078.002 (domain account) â†’ privilege_escalation
                           â””â”€ Crown Jewel: Domain Controller
```
...
**Week 2 â€” Execution (External Phase):**
...
**Week 3 â€” Execution (Internal Phase):**
...
**Week 4 â€” Reporting:**
1. Compile findings â€” techniques executed, detection rates, crown jewels reached
...
```bash
# Assumed breach â€” credentialed access starting position
python3 scripts/engagement_planner.py \
...
1. **Operating without written authorization** â€” Unauthorized red team activity against any system you don't own or have explicit permission to test is a criminal offense. The `--authorized` flag must reflect a real signed RoE, not just running the tool to bypass the check. Authorization must predate execution.
2. **Skipping kill-chain phase ordering** â€” Jumping directly to lateral movement without establishing persistence means a single detection wipes out the entire foothold. Follow the kill-chain phase order â€” each phase builds the foundation for the next.
3. **Not defining crown jewels before starting** â€” Engagements without defined success criteria drift into open-ended vulnerability hunting. Crown jewels and success conditions must be agreed upon in the RoE before the first technique is executed.
4. **Ignoring OPSEC risks in the plan** â€” Red team exercises test blue team detection. Deliberately avoiding all detectable techniques produces an unrealistic engagement that doesn't validate detection coverage. Use OPSEC risks to understand detection exposure, not to avoid it entirely.
5. **Failing to document executed techniques in real time** â€” Retroactive documentation of what was executed is unreliable. Log each technique, timestamp, and outcome as it happens. Post-engagement reporting must be based on contemporaneous records.
6. **Not cleaning up artifacts post-exercise** â€” Persistence mechanisms, new accounts, modified configurations, and staged data must be removed after engagement completion. Leaving red team artifacts creates permanent security risks and can be confused with real attacker activity.
7. **Treating path of least resistance as the only path** â€” Attackers adapt. Test multiple attack paths including higher-effort routes that may evade detection. Validating that the easiest path is detected is necessary but not sufficient.
...
| Skill | Relationship |
|-------|-------------|
| [threat-detection](https://github.com/Patasse97/claude-skills/tree/main/engineering-team/threat-detection/SKILL.md) | Red team technique execution generates realistic TTPs that validate threat hunting hypotheses |
| [incident-response](https://github.com/Patasse97/claude-skills/tree/main/engineering-team/incident-response/SKILL.md) | Red team activity should trigger incident response procedures â€” detection and response quality is a primary success metric |
| [cloud-security](https://github.com/Patasse97/claude-skills/tree/main/engineering-team/cloud-security/SKILL.md) | Cloud posture findings (IAM misconfigs, S3 exposure) become red team attack path targets |
| [security-pen-testing](https://github.com/Patasse97/claude-skills/tree/main/engineering-team/security-pen-testing/SKILL.md) | Pen testing focuses on specific vulnerability exploitation; red team focuses on end-to-end kill-chain simulation to crown jewels |
