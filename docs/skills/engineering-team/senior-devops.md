---
title: "Senior Devops â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/senior-devops/SKILL.md">Source</a></span>
</div>
...
```bash
# Script 1: Pipeline Generator â€” scaffolds CI/CD pipelines for GitHub Actions or CircleCI
python scripts/pipeline_generator.py ./app --platform=github --stages=build,test,deploy

# Script 2: Terraform Scaffolder â€” generates and validates IaC modules for AWS/GCP/Azure
python scripts/terraform_scaffolder.py ./infra --provider=aws --module=ecs-service --verbose

# Script 3: Deployment Manager â€” orchestrates container deployments with rollback support
...
**Example â€” AWS ECS service module:**
```hcl
...
**Example â€” Kubernetes blue/green deployment (blue-slot specific elements):**
```yaml
...
- Pattern Reference: `references/cicd_pipeline_guide.md` â€” detailed CI/CD patterns, best practices, anti-patterns
- Workflow Guide: `references/infrastructure_as_code.md` â€” IaC step-by-step processes, optimization, troubleshooting
- Technical Guide: `references/deployment_strategies.md` â€” deployment strategy configs, security considerations, scalability
...
# Validate and plan â€” review diff before applying
...
kubectl get pods -n production -l app=myapp
curl -sf https://app.example.com/healthz || echo "ROLLBACK FAILED â€” escalate"
```
...
- **Single-cloud** (default) â€” lower operational complexity, deeper managed-service integration, better cost leverage with committed-use discounts
- **Multi-cloud** â€” required when mandated by compliance/data residency, acquiring companies on different clouds, or needing best-of-breed services across providers (e.g., AWS for compute + GCP for ML)
- **Hybrid** â€” on-prem + cloud; use when regulated workloads must stay on-prem while burst/non-sensitive workloads run in the cloud

> Start single-cloud. Add a second cloud only when there is a concrete business or compliance driver â€” not for theoretical redundancy.
...
- Full programming language â€” loops, conditionals, unit tests native
...
| **Cloud Deployment Manager** | GCP-only; rare â€” most GCP teams prefer Terraform |
