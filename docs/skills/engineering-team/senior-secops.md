---
title: "Senior SecOps Engineer â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/senior-secops/SKILL.md">Source</a></span>
</div>
...
# STOP if exit code 2 â€” resolve critical findings before continuing
```

```bash
# Step 2: Check dependency vulnerabilities
python scripts/vulnerability_assessor.py . --severity high
# STOP if exit code 2 â€” patch critical CVEs before continuing
```

```bash
# Step 3: Verify compliance controls
python scripts/compliance_checker.py . --framework all
# STOP if exit code 2 â€” address critical gaps before proceeding
```
...
Each step fails the pipeline on its respective exit code â€” no deployment proceeds past a critical finding.
...
1. ASSESS (0-2 hours)
   ...
   - STOP if CVSS 9.0+ on internet-facing system â€” escalate immediately
...
3. REMEDIATE
   ...
   - STOP if scanner still flags the CVE â€” do not deploy
...
**Exit Codes:** `0` = no critical/high findings Â· `1` = high severity findings Â· `2` = critical severity findings
...
**Exit Codes:** `0` = no critical/high vulnerabilities Â· `1` = high severity vulnerabilities Â· `2` = critical severity vulnerabilities
...
**Exit Codes:** `0` = compliant (90%+ score) Â· `1` = non-compliant (50-69% score) Â· `2` = critical gaps (<50% score)
...
Rapid 15-minute assessment â€” run through each category and note pass/fail. For deep-dive testing, hand off to the **security-pen-testing** skill.
...
**SBOM Generation:**
- **syft** â€” generates SBOMs from container images or source dirs (SPDX, CycloneDX formats)
- **cyclonedx-cli** â€” CycloneDX-native tooling; merge multiple SBOMs for mono-repos
