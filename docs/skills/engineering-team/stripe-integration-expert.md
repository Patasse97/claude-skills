---
title: "Stripe Integration Expert â€” Agent Skill & Codex Plugin"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering-team/stripe-integration-expert/SKILL.md">Source</a></span>
</div>
...
```
FREE_TRIAL â”€â”€paidâ”€â”€â–º ACTIVE â”€â”€cancelâ”€â”€â–º CANCEL_PENDING â”€â”€period_endâ”€â”€â–º CANCELED
     â”‚                  â”‚                                                    â”‚
     â”‚               downgrade                                            reactivate
     â”‚                  â–¼                                                    â”‚
     â”‚             DOWNGRADING â”€â”€period_endâ”€â”€â–º ACTIVE (lower plan)           â”‚
     â”‚                                                                        â”‚
     â””â”€â”€trial_end without paymentâ”€â”€â–º PAST_DUE â”€â”€payment_failed 3xâ”€â”€â–º CANCELED
                                          â”‚
                                     payment_success
                                          â”‚
                                          â–¼
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Return 500 so Stripe retries â€” don't mark as processed
...
- **Webhook delivery order not guaranteed** â€” always re-fetch from Stripe API, never trust event data alone for DB updates
- **Double-processing webhooks** â€” Stripe retries on 500; always use idempotency table
- **Trial conversion tracking** â€” store `hasHadTrial: true` in DB to prevent trial abuse
- **Proration surprises** â€” always preview proration before upgrade; show user the amount before confirming
- **Customer portal not configured** â€” must enable features in Stripe dashboard under Billing â†’ Customer portal settings
- **Missing metadata on checkout** â€” always pass `userId` in metadata; can't link subscription to user without it
