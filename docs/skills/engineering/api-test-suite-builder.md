---
title: "API Test Suite Builder â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/api-test-suite-builder/SKILL.md">Source</a></span>
</div>
...
- **Route detection** â€” scan source files to extract all API endpoints
- **Auth coverage** â€” valid/invalid/expired tokens, missing auth header
- **Input validation** â€” missing fields, wrong types, boundary values, injection attempts
- **Error code matrix** â€” 400/401/403/404/422/500 for each route
- **Pagination** â€” first/last/empty/oversized pages
- **File uploads** â€” valid, oversized, wrong MIME type, empty
- **Rate limiting** â€” burst detection, per-user vs global limits

---

## When to Use

- New API added â€” generate test scaffold before writing implementation (TDD)
- Legacy API with no tests â€” scan and generate baseline coverage
- API contract review â€” verify existing tests match current route definitions
- Pre-release regression check â€” ensure all routes have at least smoke tests
- Security audit prep â€” generate adversarial input tests
...
## Example Test Files
â†’ See references/example-test-files.md for details
...
5. **Use factories/fixtures** for test data â€” never hardcode IDs
...
- **Testing only happy paths** â€” 80% of bugs live in error paths; test those first
- **Hardcoded test data IDs** â€” use factories/fixtures; IDs change between environments
- **Shared state between tests** â€” always clean up in afterEach/afterAll
- **Testing implementation, not behavior** â€” test what the API returns, not how it does it
- **Missing boundary tests** â€” off-by-one errors are extremely common in pagination and limits
- **Not testing token expiry** â€” expired tokens behave differently from invalid ones
- **Ignoring Content-Type** â€” test that API rejects wrong content types (xml when json expected)

---

## Best Practices

1. One describe block per endpoint â€” keeps failures isolated and readable
2. Seed minimal data â€” don't load the entire DB; create only what the test needs
3. Use `beforeAll` for shared setup, `afterAll` for cleanup â€” not `beforeEach` for expensive ops
...
7. Add rate limit tests last â€” they can interfere with other test suites if run in parallel
