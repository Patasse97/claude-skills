---
title: "Browser Automation â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/SKILL.md">Source</a></span>
</div>
...
- Writing browser tests or E2E test suites â€” use **playwright-pro** instead
- Testing API endpoints â€” use **api-test-suite-builder** instead
- Load testing or performance benchmarking â€” use **performance-profiler** instead

**Why Playwright over Selenium or Puppeteer:**
- **Auto-wait built in** â€” no explicit `sleep()` or `waitForElement()` needed for most actions
- **Multi-browser from one API** â€” Chromium, Firefox, WebKit with zero config changes
- **Network interception** â€” block ads, mock responses, capture API calls natively
- **Browser contexts** â€” isolated sessions without spinning up new browser instances
- **Codegen** â€” `playwright codegen` records your actions and generates scripts
- **Async-first** â€” Python async/await for high-throughput scraping
...
1. `data-testid`, `data-id`, or custom data attributes â€” stable across redesigns
2. `#id` selectors â€” unique but may change between deploys
3. Semantic selectors: `article`, `nav`, `main`, `section` â€” resilient to CSS changes
4. Class-based: `.product-card`, `.price` â€” brittle if classes are generated (e.g., CSS modules)
5. Positional: `nth-child()`, `nth-of-type()` â€” last resort, breaks on layout changes

Use XPath only when CSS cannot express the relationship (e.g., ancestor traversal, text-based selection).

**Pagination strategies:** next-button, URL-based (`?page=N`), infinite scroll, load-more button. See [data_extraction_recipes.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/data_extraction_recipes.md) for complete pagination handlers and scroll patterns.
...
See [playwright_browser_api.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/playwright_browser_api.md) for full screenshot/PDF options.

### 4. Structured Data Extraction

Core extraction patterns:
- **Tables to JSON** â€” Extract `<thead>` headers and `<tbody>` rows into dictionaries
- **Listings to arrays** â€” Map repeating card elements using a field-selector map (supports `::attr()` for attributes)
- **Nested/threaded data** â€” Recursive extraction for comments with replies, category trees

See [data_extraction_recipes.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/data_extraction_recipes.md) for complete extraction functions, price parsing, data cleaning utilities, and output format helpers (JSON, CSV, JSONL).
...
**Best practice:** Save state after login, reuse across scraping sessions. Check session validity before starting a long job â€” make a lightweight request to a protected page and verify you are not redirected to login. See [playwright_browser_api.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/playwright_browser_api.md) for cookie and storage state API details.

### 6. Anti-Detection Patterns

Modern websites detect automation through multiple vectors. Apply these in priority order:

1. **WebDriver flag removal** â€” Remove `navigator.webdriver = true` via init script (critical)
2. **Custom user agent** â€” Rotate through real browser UAs; never use the default headless UA
3. **Realistic viewport** â€” Set 1920x1080 or similar real-world dimensions (default 800x600 is a red flag)
4. **Request throttling** â€” Add `random.uniform()` delays between actions
5. **Proxy support** â€” Per-browser or per-context proxy configuration

See [anti_detection_patterns.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/anti_detection_patterns.md) for the complete stealth stack: navigator property hardening, WebGL/canvas fingerprint evasion, behavioral simulation (mouse movement, typing speed, scroll patterns), proxy rotation strategies, and detection self-test URLs.
...
See [playwright_browser_api.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/playwright_browser_api.md) for wait strategies, network interception, and Shadow DOM details.
...
See [anti_detection_patterns.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/browser-automation/references/anti_detection_patterns.md) for the complete exponential backoff implementation and rate limiter class.
...
- **playwright-pro** â€” Browser testing skill. Use for E2E tests, test assertions, test fixtures. Browser Automation is for data extraction and workflow automation, not testing.
- **api-test-suite-builder** â€” When the website has a public API, hit the API directly instead of scraping the rendered page. Faster, more reliable, less detectable.
- **performance-profiler** â€” If your automation scripts are slow, profile the bottlenecks before adding concurrency.
- **env-secrets-manager** â€” For securely managing credentials used in authenticated automation workflows.
