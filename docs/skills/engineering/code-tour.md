---
title: "Code Tour â€” Agent Skill for Codex & OpenClaw"
description: "Use when the user asks to create a CodeTour .tour file â€” persona-targeted, step-by-step walkthroughs that link to real files and line numbers. Agent skill for Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/code-tour/SKILL.md">Source</a></span>
</div>
...
Create **CodeTour** files â€” persona-targeted, step-by-step walkthroughs of a codebase that link directly to files and line numbers. CodeTour files live in `.tours/` and work with the [VS Code CodeTour extension](https://github.com/microsoft/codetour).

## Overview

A great tour is a **narrative** â€” a story told to a specific person about what matters, why it matters, and what to do next. Only create `.tour` JSON files. Never modify source code.
...
Then: identify language(s), framework(s), project purpose. Map folder structure 1-2 levels deep. Find entry points â€” every path in the tour must be real.

If the repo has fewer than 5 source files, create a quick-depth tour regardless of persona â€” there's not enough to warrant a deep one.
...
When intent is ambiguous, default to **new-joiner** persona at **standard** depth â€” it's the most generally useful.
...
```json
{
  ...
  "title": "Descriptive Title â€” Persona / Goal",
...
### Writing descriptions â€” SMIG formula

- **S â€” Situation**: What is the reader looking at?
- **M â€” Mechanism**: How does this code work?
- **I â€” Implication**: Why does this matter for this persona?
- **G â€” Gotcha**: What would a smart person get wrong?
...
1. **Orientation** â€” `file` or `directory` step (never content-only first step â€” blank in VS Code)
2. **High-level map** â€” 1-3 directory steps showing major modules
3. **Core path** â€” file/line steps, the heart of the tour
4. **Closing** â€” what the reader can now do, suggested follow-ups

## Anti-Patterns

| Anti-pattern | Fix |
|---|---|
| **File listing** â€” "this file contains the models" | Tell a story. Each step depends on the previous. |
...
| **Recap closing** â€” "we covered X, Y, Z" | Tell the reader what they can now *do*. |
| **Content-only first step** | Anchor step 1 to a file or directory. |

## Cross-References

- Related: `engineering/codebase-onboarding` â€” for broader onboarding beyond tours
- Related: `engineering/pr-review-expert` â€” for automated PR review workflows
