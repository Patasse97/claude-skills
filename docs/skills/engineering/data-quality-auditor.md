---
title: "Profile from CSV â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/data-quality-auditor/SKILL.md">Source</a></span>
</div>
...
### Mode 1 â€” Full Audit (New Dataset)
Use when you have a dataset you've never assessed before.

1. **Profile** â€” Run `data_profiler.py` to get shape, types, completeness, and distributions
2. **Missing Values** â€” Run `missing_value_analyzer.py` to classify missingness patterns (MCAR/MAR/MNAR)
3. **Outliers** â€” Run `outlier_detector.py` to flag anomalies using IQR and Z-score methods
4. **Cross-column checks** â€” Inspect referential integrity, duplicate rows, and logical constraints
5. **Score & Report** â€” Assign a Data Quality Score (DQS) and produce the remediation plan

### Mode 2 â€” Targeted Scan (Specific Concern)
...
### Mode 3 â€” Ongoing Monitoring Setup
Use when the user wants recurring quality checks on a live pipeline.

1. Identify the 5â€“8 critical columns driving key metrics
...
- Outputs a DQS (0â€“100) based on completeness + consistency signals
...
The DQS is a 0â€“100 composite score across five dimensions. Report it at the top of every audit.
...
**Scoring thresholds:**
- ðŸŸ¢ 85â€“100 â€” Production-ready
- ðŸŸ¡ 65â€“84 â€” Usable with documented caveats
- ðŸ”´ 0â€“64 â€” Remediation required before use
...
- **Silent nulls** â€” Nulls encoded as `0`, `""`, `"N/A"`, `"null"` strings. Completeness metrics lie until these are caught.
- **Leaky timestamps** â€” Future dates, dates before system launch, or timezone mismatches that corrupt time-series joins.
- **Cardinality explosions** â€” Free-text fields with thousands of unique values masquerading as categorical. Will break one-hot encoding silently.
- **Duplicate keys** â€” PKs that aren't unique invalidate joins and aggregations downstream.
- **Distribution shift** â€” Columns where current distribution diverges from baseline (>2Ïƒ on mean/std). Signals upstream pipeline changes.
- **Correlated missingness** â€” Nulls concentrated in a specific time range, user segment, or region â€” evidence of MNAR, not random dropout.
...
| 1â€“10% | Impute; add a binary indicator column `col_was_null` |
| 10â€“30% | Impute cautiously; investigate root cause; document assumption |
...
- ðŸŸ¢ **Verified** â€” confirmed by data inspection or domain owner
- ðŸŸ¡ **Likely** â€” strong signal but not fully confirmed
- ðŸ”´ **Assumed** â€” inferred from patterns; needs domain validation

Never auto-remediate ðŸ”´ findings without human confirmation.
...
**Bottom Line** â€” DQS score and one-sentence verdict (e.g., "DQS: 61/100 â€” remediation required before production use")
**What** â€” The specific issues found (ranked by severity Ã— breadth)
**Why It Matters** â€” Business or analytical impact of each issue
**How to Act** â€” Specific, ordered remediation steps
...
- You need to design or optimize the database schema â€” use `engineering/database-designer`
- You need to build the ETL pipeline itself â€” use an engineering skill
- The dataset is a financial model output â€” use `finance/financial-analyst` for model validation

---

## References

- `references/data-quality-concepts.md` â€” MCAR/MAR/MNAR theory, DQS methodology, outlier detection methods
