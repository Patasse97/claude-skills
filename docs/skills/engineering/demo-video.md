---
title: "Demo Video â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/demo-video/SKILL.md">Source</a></span>
</div>
...
Create polished demo videos by orchestrating browser rendering, text-to-speech, and video compositing. Think like a video producer â€” story arc, pacing, emotion, visual hierarchy. Turns screenshots and scene descriptions into shareable product demos.
...
- **playwright MCP available?** â€” needed for automated screenshots. Fallback: ask user to screenshot the HTML files manually.
- **edge-tts available?** â€” needed for narration audio. Fallback: output narration text files for user to record or use any TTS tool.
- **ffmpeg available?** â€” needed for compositing. Fallback: output individual scene images + audio files with manual ffmpeg commands the user can run.
...
| Mode | How | When |
|------|-----|------|
| **MCP Orchestration** | HTML â†’ playwright screenshots â†’ edge-tts audio â†’ ffmpeg composite | Use when playwright + edge-tts + ffmpeg MCPs are all connected |
...
1. `scenes/` â€” one HTML file per scene (1920x1080 viewport)
2. `narration/` â€” one `.txt` file per scene (for edge-tts input)
3. `scenes.json` â€” manifest listing scenes in order with durations and narration text
4. `build.sh` â€” shell script that runs the full pipeline:
   - `playwright screenshot` each HTML scene â†’ `frames/`
   - `edge-tts` each narration file â†’ `audio/`
   - `ffmpeg` concat with crossfade transitions â†’ `output.mp4`

If MCPs are unavailable, still produce items 1-3. Include the ffmpeg commands in `build.sh` for the user to run manually.

## Scene Design System

See [references/scene-design-system.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/demo-video/references/scene-design-system.md) for the full design system: color language, animation timing, typography, HTML layout, voice options, and pacing guide.
...
| Anti-pattern | Fix |
|---|---|
| **Slideshow pacing** â€” every scene same duration, no rhythm | Vary durations: hooks 3s, proof 8s, CTA 4s |
| **Wall of text on screen** | Move info to narration, simplify visuals |
| **Generic narration** â€” "This feature lets you..." | Use specific numbers and concrete verbs |
| **No story arc** â€” just listing features | Use problem -> solution -> proof structure |
...
- Related: `engineering/browser-automation` â€” for playwright-based browser workflows
- See also: [framecraft](https://github.com/vaddisrinivas/framecraft) â€” open-source scene rendering pipeline
