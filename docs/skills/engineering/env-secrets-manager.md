---
title: "Env & Secrets Manager â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/env-secrets-manager/SKILL.md">Source</a></span>
</div>
...
- **Single cloud provider** â€” use the cloud-native secret manager. It integrates tightly with IAM, reduces operational overhead, and costs less than self-hosting.
- **Multi-cloud or hybrid** â€” use HashiCorp Vault. It provides a uniform API across environments and supports dynamic secret generation (database credentials, cloud IAM keys) that expire automatically.
- **Kubernetes-heavy** â€” combine External Secrets Operator with any backend above to sync secrets into K8s `Secret` objects without hardcoding.

### Application Access Patterns

1. **SDK/API pull** â€” application fetches secret at startup or on-demand via provider SDK.
2. **Sidecar injection** â€” a sidecar container (e.g., Vault Agent) writes secrets to a shared volume or injects them as environment variables.
3. **Init container** â€” a Kubernetes init container fetches secrets before the main container starts.
4. **CSI driver** â€” secrets mount as a filesystem volume via the Secrets Store CSI Driver.
...
- **AWS Secrets Manager** â€” use built-in Lambda-based rotation for RDS, Redshift, and DocumentDB.
- **HashiCorp Vault** â€” configure dynamic secrets with TTLs; credentials are generated on-demand and auto-expire.
- **Azure Key Vault** â€” use Event Grid notifications to trigger rotation functions.
- **GCP Secret Manager** â€” use Pub/Sub notifications tied to Cloud Functions for rotation logic.
...
- **Restrict PR access** â€” do not expose secrets to pipelines triggered by forks or untrusted branches.
...
```toml
# .gitleaks.toml â€” minimal configuration
[extend]
useDefault = true
...
- Review false positive lists during security audits â€” patterns may mask real leaks over time.
