---
title: "PR Review Expert â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/pr-review-expert/SKILL.md">Source</a></span>
</div>
...
Structured, systematic code review for GitHub PRs and GitLab MRs. Goes beyond style nits â€” this skill
...
- **Blast radius analysis** â€” trace which files, services, and downstream consumers could break
- **Security scan** â€” SQL injection, XSS, auth bypass, secret exposure, dependency vulns
- **Test coverage delta** â€” new code vs new tests ratio
- **Breaking change detection** â€” API contracts, DB schema migrations, config keys
- **Ticket linking** â€” verify Jira/Linear ticket exists and matches scope
- **Performance impact** â€” N+1 queries, bundle size regression, memory allocations
...
- After an incident â€” review similar PRs proactively
...
### Step 2 â€” Blast Radius Analysis

For each changed file, identify:

1. **Direct dependents** â€” who imports this file?
```bash
...
2. **Service boundaries** â€” does this change cross a service?
```bash
# Check if changed files span multiple services (monorepo)
gh pr diff $PR --name-only | cut -d/ -f1-2 | sort -u
```

3. **Shared contracts** â€” types, interfaces, schemas
```bash
gh pr diff $PR --name-only | grep -E "types/|interfaces/|schemas/|models/"
```

**Blast radius severity:**
- CRITICAL â€” shared library, DB model, auth middleware, API contract
- HIGH     â€” service used by >3 others, shared config, env vars
- MEDIUM   â€” single service internal change, utility function
- LOW      â€” UI component, test file, docs

### Step 3 â€” Security Scan

```bash
DIFF=/tmp/pr-$PR.diff

# SQL Injection â€” raw query string interpolation
...
- New function without tests â†’ flag
- Deleted tests without deleted code â†’ flag
- Coverage drop >5% â†’ block merge
- Auth/payments paths â†’ require 100% coverage

### Step 5 â€” Breaking Change Detection
...
Blast Radius: HIGH â€” changes lib/auth used by 5 services
...
3. N+1 pattern in src/services/reports.ts:88
   findUser() called inside results.map() â€” batch with findManyUsers(ids)
...
- **Reviewing style over substance** â€” let the linter handle style; focus on logic, security, correctness
- **Missing blast radius** â€” a 5-line change in a shared utility can break 20 services
- **Approving untested happy paths** â€” always verify error paths have coverage
- **Ignoring migration risk** â€” NOT NULL additions need a default or two-phase migration
- **Indirect secret exposure** â€” secrets in error messages/logs, not just hardcoded values
- **Skipping large PRs** â€” if a PR is too large to review properly, request it be split

---

## Best Practices

1. Read the linked ticket before looking at code â€” context prevents false positives
2. Check CI status before reviewing â€” don't review code that fails to build
...
6. Batch all comments in one review round â€” don't trickle feedback
7. Acknowledge good patterns, not just problems â€” specific praise improves culture
