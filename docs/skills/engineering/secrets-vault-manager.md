---
title: "Secrets Vault Manager â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/secrets-vault-manager/SKILL.md">Source</a></span>
</div>
...
**Distinct from env-secrets-manager** which handles local `.env` file hygiene and leak detection. This skill operates at the infrastructure layer â€” Vault clusters, cloud KMS, certificate authorities, and CI/CD secret injection.
...
| Audit devices | File + syslog (dual) | Vault refuses requests if all audit devices fail â€” dual prevents outages |

### Auth Methods

**AppRole** â€” Machine-to-machine authentication for services and batch jobs.
...
**Kubernetes** â€” Pod-native authentication via service account tokens.
...
**OIDC** â€” Human operator access via SSO provider (Okta, Azure AD, Google Workspace).
...
**Principle:** Always fetch secrets at startup or via sidecar â€” never bake into images or config files.
...
3. Deploy applications â€” they read `current`
...
Vault can generate short-lived AWS IAM credentials, Azure service principal passwords, or GCP service account keys â€” eliminating long-lived cloud credentials entirely.
...
3. SSH servers trust the CA public key â€” no `authorized_keys` management
4. Certificates expire automatically â€” no revocation needed for normal operations
...
1. **Access inventory** â€” Which identities accessed which secrets, when
2. **Rotation compliance** â€” Secrets overdue for rotation
3. **Policy drift** â€” Policies modified since last review
4. **Orphaned secrets** â€” Secrets with no recent access (>90 days)
...
1. **Identify scope** â€” Which secret(s) leaked, where (repo, log, error message, third party)
2. **Revoke immediately** â€” Rotate the compromised credential at the source (provider API, Vault, cloud SM)
3. **Invalidate tokens** â€” Revoke all Vault tokens that accessed the leaked secret
4. **Audit blast radius** â€” Query audit logs for usage of the compromised secret in the exposure window
5. **Notify stakeholders** â€” Security team, affected service owners, compliance (if PII/regulated data)
6. **Post-mortem** â€” Document root cause, update controls to prevent recurrence
...
- **env-secrets-manager** â€” Local `.env` file hygiene, leak detection, drift awareness
- **senior-secops** â€” Security operations, incident response, threat modeling
- **ci-cd-pipeline-builder** â€” Pipeline design where secrets are consumed
- **docker-development** â€” Container secret injection patterns
- **helm-chart-builder** â€” Kubernetes secret management in Helm charts
