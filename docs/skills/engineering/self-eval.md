---
title: "Self-Eval: Honest Work Evaluation â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/self-eval/SKILL.md">Source</a></span>
</div>
...
- **Two-axis scoring** â€” Independently rates task ambition (Low/Medium/High) and execution quality (Poor/Adequate/Strong), then combines via a lookup matrix
- **Mandatory devil's advocate** â€” Before finalizing, must argue for both higher AND lower scores, then resolve the tension
- **Score persistence** â€” Appends scores to `.self-eval-scores.jsonl` in the working directory, building history across sessions
- **Anti-inflation detection** â€” Reads past scores and flags clustering (4+ of last 5 identical)
- **Matrix-locked scoring** â€” The composite score comes from the matrix, not from direct selection. Low ambition caps at 2/5 regardless of execution quality
...
**Ambition:** Medium â€” New feature with established patterns, but required careful backwards compatibility
**Execution:** Strong â€” All tests pass, handles edge cases, existing clients unaffected
...
- Resolution: Medium ambition holds â€” the backwards-compat constraint added real complexity beyond copy-paste.

**Score: 4** â€” Solid execution on meaningful work with real constraints.
```
...
**Ambition:** Medium â€” Required debugging across 3 services to find the root cause
**Execution:** Adequate â€” Fix works but the test only covers the exact scenario reported, not the general class of race conditions
...
- Resolution: Medium ambition is correct for the investigation, but execution drops to Adequate â€” a more thorough fix would address the pattern, not just the instance.

**Score: 3** â€” Good debugging work but the fix is narrow.
```
...
## How to Score â€” Two-Axis Model

Score on two independent axes, then combine using the matrix. Do NOT pick a number first and rationalize it â€” rate each axis separately, then read the matrix.

### Axis 1: Task Ambition (what was attempted)

Rate the difficulty and risk of what was worked on. NOT how well it was done.

- **Low (1)** â€” Safe, familiar, routine. No real risk of failure. Examples: minor config changes, simple refactors, copy-paste with small modifications, tasks you were confident you'd complete before starting.
- **Medium (2)** â€” Meaningful work with novelty or challenge. Partial failure was possible. Examples: new feature implementation, integrating an unfamiliar API, architectural changes, debugging a tricky issue.
- **High (3)** â€” Ambitious, unfamiliar, or high-stakes. Real risk of complete failure. Examples: building something from scratch in an unfamiliar domain, complex system redesign, performance-critical optimization, shipping to production under pressure.
...
- **Poor (1)** â€” Major failures, incomplete, wrong output, or abandoned mid-task. The deliverable doesn't meet its own stated criteria.
- **Adequate (2)** â€” Completed but with gaps, shortcuts, or missing rigor. Did the thing but left obvious improvements on the table.
- **Strong (3)** â€” Well-executed, thorough, quality output. No obvious improvements left undone given the scope.
...
**Read the matrix, don't override it.** The composite is your score. The devil's advocate below can cause you to re-rate an axis â€” but you cannot directly override the matrix result.
...
If your devil's advocate is less than 3 sentences total, you're not engaging with it â€” try harder.
...
**Ambition:** [Low/Medium/High] â€” [1-sentence justification]
**Execution:** [Poor/Adequate/Strong] â€” [1-sentence justification]
...
**Score: [1-5]** â€” [1-sentence final justification]
