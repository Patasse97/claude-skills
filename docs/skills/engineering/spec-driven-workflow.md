---
title: "Spec-Driven Workflow â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/spec-driven-workflow/SKILL.md">Source</a></span>
</div>
...
4. **Creates accountability.** The spec is the definition of done. No arguments about whether a feature is "complete" â€” either it satisfies the acceptance criteria or it does not.
...
Every spec follows this structure. No sections are optional â€” if a section does not apply, write "N/A â€” [reason]" so reviewers know it was considered, not forgotten.
...
| 4 | **Non-Functional Requirements** | Performance, security, accessibility, scalability, reliability â€” all with measurable thresholds. |
...
See [spec_format_guide.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/spec-driven-workflow/references/spec_format_guide.md) for the complete template with section-by-section examples, good/bad requirement patterns, and feature-type templates (CRUD, Integration, Migration).

See [acceptance_criteria_patterns.md](https://github.com/Patasse97/claude-skills/tree/main/engineering/spec-driven-workflow/references/acceptance_criteria_patterns.md) for a full pattern library of Given/When/Then criteria across authentication, CRUD, search, file upload, payment, notification, and accessibility scenarios.
...
3. **Changes are non-breaking** â€” no public API, schema, or behavior changes.
...
**Question:** [Specific, answerable question â€” not "what should I do?"]
**Options considered:**
  A. [Option] â€” Pros: [...] Cons: [...]
  B. [Option] â€” Pros: [...] Cons: [...]
...
3. Tests are stubs â€” they define the assertion but not the implementation.
...
3. Run the full test suite â€” no regressions.
4. Commit.
...
- [ ] **API contracts match implementation.** Request/response shapes in code match the spec exactly. Field names, types, status codes â€” all of it.
...
Phase 2: Write Spec
Phase 3: Validate Spec
Phase 4: Generate Tests  â”€â”€â†’  RED: Tests exist and fail
Phase 5: Implement       â”€â”€â†’  GREEN: Minimal code to pass
Phase 6: Self-Review     â”€â”€â†’  REFACTOR: Clean up internals
```
...
- **`engineering-team/tdd-guide`** â€” Red-green-refactor cycle, test generation, coverage analysis. Use after Phase 4 of this workflow.
- **`engineering/focused-fix`** â€” Deep-dive feature repair. When a spec-driven implementation has systemic issues, use focused-fix for diagnosis.
- **`engineering/rag-architect`** â€” If the feature involves retrieval or knowledge systems, use rag-architect for the technical design within the spec.
- **`references/spec_format_guide.md`** â€” Complete template with section-by-section explanations.
- **`references/bounded_autonomy_rules.md`** â€” Full decision matrix for when to stop vs. continue.
- **`references/acceptance_criteria_patterns.md`** â€” Pattern library for writing Given/When/Then criteria.
