---
title: "SQL Database Assistant - POWERFUL Tier Skill â€” Agent Skill for Codex & OpenClaw"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/engineering/sql-database-assistant/SKILL.md">Source</a></span>
</div>
...
- **Natural Language to SQL** â€” translate requirements into correct, performant queries
- **Schema Exploration** â€” introspect live databases across PostgreSQL, MySQL, SQLite, SQL Server
- **Query Optimization** â€” EXPLAIN analysis, index recommendations, N+1 detection, rewrite patterns
- **Migration Generation** â€” up/down scripts, zero-downtime strategies, rollback plans
- **ORM Integration** â€” Prisma, Drizzle, TypeORM, SQLAlchemy patterns and escape hatches
- **Multi-Database Support** â€” dialect-aware SQL with compatibility guidance
...
1. **Identify entities** â€” map nouns to tables
2. **Identify relationships** â€” map verbs to JOINs or subqueries
3. **Identify filters** â€” map adjectives/conditions to WHERE clauses
4. **Identify aggregations** â€” map "total", "average", "count" to GROUP BY
5. **Identify ordering** â€” map "top", "latest", "highest" to ORDER BY + LIMIT
...
**PostgreSQL â€” list tables and columns**
```sql
...
**SQLite â€” schema dump**
```sql
SELECT name, sql FROM sqlite_master WHERE type = 'table' ORDER BY name;
```

**SQL Server â€” columns with types**
```sql
...
2. **Identify the costliest node** â€” Seq Scan on large tables, Nested Loop with high row estimates
3. **Check for missing indexes** â€” sequential scans on filtered columns
4. **Look for estimation errors** â€” planned vs actual rows divergence signals stale statistics
5. **Evaluate JOIN order** â€” ensure the smallest result set drives the join
...
- **Batch updates** â€” process in chunks of 1000-10000 rows to avoid lock contention
- **Background jobs** â€” run backfills asynchronously with progress tracking
- **Dual-write** â€” write to old and new columns during transition period
- **Validation queries** â€” verify row counts and data integrity after each batch

### Rollback Strategies

Every migration must have a reversible down script. For irreversible changes:

1. **Backup before execution** â€” `pg_dump` the affected tables
2. **Feature flags** â€” application can switch between old/new schema reads
3. **Shadow tables** â€” keep a copy of the original table during migration window
...
- **Always use parameterized queries** â€” prevents SQL injection across all dialects
- **Avoid dialect-specific functions in shared code** â€” wrap in adapter layer
- **Test migrations on target engine** â€” `information_schema` varies between engines
- **Use ISO date format** â€” `'YYYY-MM-DD'` works everywhere
- **Quote identifiers** â€” use double quotes (SQL standard) or backticks (MySQL)
...
- **Primary keys** â€” every table must have one; prefer surrogate keys (serial/UUID)
- **Foreign keys** â€” enforce referential integrity; define ON DELETE behavior explicitly
- **UNIQUE constraints** â€” for business-level uniqueness (email, slug, API key)
- **CHECK constraints** â€” validate ranges, enums, and business rules at the DB level
- **NOT NULL** â€” default to NOT NULL; make nullable only when genuinely optional
...
1. **Consistent lock ordering** â€” always acquire locks in the same table/row order
2. **Short transactions** â€” minimize time between first lock and commit
3. **Advisory locks** â€” use `pg_advisory_lock()` for application-level coordination
4. **Retry logic** â€” catch deadlock errors and retry with exponential backoff
...
- **Automate** â€” cron or systemd timer, never manual-only
- **Test restores** â€” untested backups are not backups
- **Offsite copies** â€” S3, GCS, or separate region
- **Retention policy** â€” daily for 7 days, weekly for 4 weeks, monthly for 12 months
- **Monitor backup size and duration** â€” sudden changes signal issues
