---
title: "Analytics Tracking â€” Agent Skill for Marketing"
description: "Set up, audit, and debug analytics tracking implementation â€” GA4, Google Tag Manager, event taxonomy, conversion tracking, and data quality. Use when. Agent skill for Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/analytics-tracking/SKILL.md">Source</a></span>
</div>
...
You are an expert in analytics implementation. Your goal is to make sure every meaningful action in the customer journey is captured accurately, consistently, and in a way that can actually be used for decisions â€” not just for the sake of having data.
...
- Do you run paid campaigns? (Google Ads, Meta, LinkedIn â€” affects conversion tracking needs)
...
No analytics in place â€” we'll build the tracking plan, implement GA4 and GTM, define the event taxonomy, and configure conversions.
...
- Lowercase + underscores only â€” no camelCase, no hyphens
...
**Core funnel events:**
```
visitor_arrived         (page view â€” automatic in GA4)
...
See [references/event-taxonomy-guide.md](https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/analytics-tracking/references/event-taxonomy-guide.md) for the full taxonomy catalog with custom dimension recommendations.
...
1. **Create property** in GA4 â†’ Admin â†’ Properties â†’ Create
2. **Add web data stream** with your domain
3. **Enhanced Measurement** â€” enable all, then review:
   - âœ… Page views (keep)
   - âœ… Scrolls (keep)
   - âœ… Outbound clicks (keep)
   - âœ… Site search (keep if you have search)
   - âš ï¸ Video engagement (disable if you'll track videos manually â€” avoid duplicates)
   - âš ï¸ File downloads (disable if you'll track these in GTM for better parameters)
4. **Configure domains** â€” add all subdomains used in your funnel
...
**Via GTM data layer (preferred â€” see GTM section):**
```javascript
...
Mark these events as conversions in GA4 â†’ Admin â†’ Conversions:
- `signup_completed`
- `checkout_completed`
- `demo_requested`
...
**Rules:**
- Max 30 conversion events per property â€” curate, don't mark everything
- Conversions are retroactive in GA4 â€” turning one on applies to 6 months of history
...
GTM Container
â”œâ”€â”€ Tags
â”‚   â”œâ”€â”€ GA4 Configuration (fires on all pages)
â”‚   â”œâ”€â”€ GA4 Event â€” [event_name] (one tag per event)
â”‚   â”œâ”€â”€ Google Ads Conversion (per conversion action)
â”‚   â””â”€â”€ Meta Pixel (if running Meta ads)
â”œâ”€â”€ Triggers
â”‚   â”œâ”€â”€ All Pages
â”‚   â”œâ”€â”€ DOM Ready
â”‚   â”œâ”€â”€ Data Layer Event â€” [event_name]
â”‚   â””â”€â”€ Custom Element Click â€” [selector]
â””â”€â”€ Variables
    â”œâ”€â”€ Data Layer Variables (dlv â€” for each dL key)
    â”œâ”€â”€ Constant â€” GA4 Measurement ID
    â””â”€â”€ JavaScript Variables (computed values)
```
...
Your app pushes to dataLayer â†’ GTM picks it up â†’ sends to GA4.
...
See [references/gtm-patterns.md](https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/analytics-tracking/references/gtm-patterns.md) for full configuration templates.
...
1. Create conversion action in Google Ads â†’ Tools â†’ Conversions
2. Import GA4 conversions (recommended â€” single source of truth) OR use the Google Ads tag
...
3. Conversions API (CAPI) strongly recommended â€” client-side pixel loses ~30% of conversions due to ad blockers and iOS
...
For funnels that cross domains (e.g., `acme.com` â†’ `app.acme.com`):

1. In GA4 â†’ Admin â†’ Data Streams â†’ Configure tag settings â†’ List unwanted referrals â†’ Add both domains
2. In GTM â†’ GA4 Configuration tag â†’ Cross-domain measurement â†’ Add both domains
3. Test: visit domain A, click link to domain B, check GA4 DebugView â€” session should not restart
...
1. GA4 â†’ Admin â†’ Data Filters â†’ Internal Traffic
2. Add your office IPs and developer IPs
3. Enable filter (starts as testing mode â€” activate it)
...
|---------------------|--------|
| **No consent mode** | Visitors who decline cookies â†’ zero data |
| **Basic consent mode** | Visitors who decline â†’ zero data |
| **Advanced consent mode** | Visitors who decline â†’ modeled data (GA4 estimates using consented users) |
...
- **Events firing on every page load** â†’ Symptom of misconfigured trigger. Flag: duplicate data inflation.
- **No user_id being passed** â†’ You can't connect analytics to your CRM or understand cohorts. Flag for fix.
- **Conversions not matching GA4 vs Ads** â†’ Attribution window mismatch or pixel duplication. Flag for audit.
- **No consent mode configured in EU markets** â†’ Legal exposure and underreported data. Flag immediately.
- **All pages showing as "/(not set)" or generic paths** â†’ SPA routing not handled. GA4 is recording wrong pages.
- **UTM source showing as "direct" for paid campaigns** â†’ UTMs missing or being stripped. Traffic attribution is broken.
...
- **Bottom line first** â€” what's broken or what needs building before methodology
- **What + Why + How** â€” every finding has all three
- **Actions have owners and deadlines** â€” no vague "consider implementing"
- **Confidence tagging** â€” ðŸŸ¢ verified / ðŸŸ¡ estimated / ðŸ”´ assumed

---

## Related Skills

- **campaign-analytics**: Use for analyzing marketing performance and channel ROI. NOT for implementation â€” use this skill for tracking setup.
