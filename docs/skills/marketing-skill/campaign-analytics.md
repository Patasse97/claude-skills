---
title: "Campaign Analytics â€” Agent Skill for Marketing"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/campaign-analytics/SKILL.md">Source</a></span>
</div>
...
- **Missing required keys** (e.g., `journeys`, `funnel.stages`, `campaigns`) â†’ script exits with a descriptive `KeyError`
- **Mismatched array lengths** in funnel data (`stages` and `counts` must be the same length) â†’ raises `ValueError`
- **Non-numeric monetary values** in ROI data â†’ raises `TypeError`
...
```bash
# Step 1 â€” Attribution: understand which channels drive conversions
python scripts/attribution_analyzer.py campaign_data.json --model time-decay

# Step 2 â€” Funnel: identify where prospects drop off on the path to conversion
python scripts/funnel_analyzer.py funnel_data.json

# Step 3 â€” ROI: calculate profitability and benchmark against industry standards
