---
title: "Churn Prevention â€” Agent Skill for Marketing"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/churn-prevention/SKILL.md">Source</a></span>
</div>
...
Starting from scratch â€” no cancel flow exists, or cancellation is immediate. We'll design the full flow from trigger to post-cancel.
...
A cancel flow is not a dark pattern â€” it's a structured conversation. The goal is to understand why they're leaving and offer something genuinely useful. If they still want to cancel, let them.

### The 5-Stage Flow

```
[Cancel Trigger] â†’ [Exit Survey] â†’ [Dynamic Save Offer] â†’ [Confirmation] â†’ [Post-Cancel]
```

**Stage 1 â€” Cancel Trigger**
- Show cancel option clearly (no hiding it â€” dark patterns burn trust)
- At the moment they click cancel, begin the flow â€” don't take them to a dead-end form
- Mobile: make this work on touch

**Stage 2 â€” Exit Survey (1 question, required)**
- Ask ONE question: "What's the main reason you're cancelling?"
- Keep it multiple choice (6-8 reasons max) â€” open text is optional, not required
- This answer drives the save offer â€” it must be collected before showing the offer

**Stage 3 â€” Dynamic Save Offer**
- Match the offer to the reason (see Exit Survey â†’ Save Offer Mapping below)
- Don't show a generic discount â€” it signals your pricing was fake
- One offer per attempt. If they decline, let them cancel.

**Stage 4 â€” Confirmation**
- Clear summary of what happens when they cancel (access, data, billing)
- Explicit confirmation button â€” "Yes, cancel my account"
- No pre-checked boxes, no confusing language

**Stage 5 â€” Post-Cancel**
...
| Just testing / never needed | No offer â€” let go | Wrong fit |
...
- One clear headline: "Before you go â€” [offer]"
...
See [references/cancel-flow-playbook.md](https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/churn-prevention/references/cancel-flow-playbook.md) for full decision trees and flow templates.
...
**1. Smart Retry Logic**
Don't retry immediately â€” failed cards often recover within 3-7 days:
...
- These update expired/replaced cards before the next charge â€” use them
...
- No guilt. No shame. Card failures happen â€” treat customers like adults.
- Every email links directly to the payment update page â€” not the dashboard

See [references/dunning-guide.md](https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/churn-prevention/references/dunning-guide.md) for full email sequences and retry configuration examples.
...
**Red flags:**
- Save rate <5% â†’ offers aren't matching reasons
- Exit survey completion <70% â†’ survey is too long or optional
- Recovery rate <20% â†’ retry logic or emails need work
...
- **Instant cancellation flow** â†’ Revenue is leaking immediately. Any friction saves money â€” flag for priority fix.
- **Single generic save offer** â†’ A discount shown to everyone depresses average revenue and trains customers to wait for deals. Map offers to exit reasons.
- **No dunning sequence** â†’ If payment fails and nothing happens, that's 20-40% of churn going unaddressed. Flag immediately.
- **Exit survey is optional** â†’ <70% completion = bad data. Make it required (one question, fast).
- **No post-cancel reactivation email** â†’ The 7-day window is the highest win-back moment. Missing it leaves money on the table.
- **Churn rate >5% monthly** â†’ At this rate, the company is likely contracting. Churn prevention alone won't fix it â€” flag for product/ICP review alongside retention work.
...
| "Model churn impact" | Run churn_impact_calculator.py with your inputs â€” monthly MRR saved and annual impact |
...
- **Bottom line first** â€” save rate estimate or recovery potential before methodology
- **What + Why + How** â€” every recommendation has all three
- **Actions have owners and deadlines** â€” no vague suggestions
