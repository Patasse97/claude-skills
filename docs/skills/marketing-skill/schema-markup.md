---
title: "Schema Markup Implementation â€” Agent Skill for Marketing"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/schema-markup/SKILL.md">Source</a></span>
</div>
...
2. Review Google Search Console â†’ Enhancements â†’ check all schema error reports
...
When they need to add structured data to pages â€” from scratch or to a new page type.
...
Pick the right schema for the page â€” stacking compatible types is fine, but don't add schema that doesn't match the page content.
...
- Never add `Product` to a page that doesn't sell a product â€” Google will penalize misuse
...
Multiple schema blocks per page are fine â€” use separate `<script>` tags or nest them in an array.
...
These are the ones that actually matter â€” the errors that kill rich results eligibility:
...
| `name` field is empty or generic | Fails validation | Use real, specific values â€” not "" or "N/A" |
| `image` URL is relative path | Invalid â€” must be absolute | Use `https://example.com/image.jpg` not `/image.jpg` |
...
| Using deprecated properties | Ignored by validators | Cross-check against current schema.org â€” types evolve |
...
This is increasingly the reason to care about schema â€” not just Google rich results.
...
- **AI systems parse your content type** â€” they know it's a HowTo vs an opinion piece vs a product listing
- **FAQPage schema increases citation likelihood** â€” AI systems love structured Q&A they can pull directly
- **Article schema with `author` and `datePublished`** â€” helps AI systems assess freshness and authority
- **Organization schema with `sameAs` links** â€” connects your entity across the web, boosting entity recognition

Practical actions for AI search visibility:
1. Add FAQPage schema to any page with Q&A content â€” even if it's just 3 questions
...
4. Keep `datePublished` and `dateModified` accurate â€” AI systems filter by freshness
...
3. **`scripts/schema_validator.py`** â€” run locally on any HTML file
...
- **FAQPage schema missing from FAQ content** â†’ any page with Q&A format and no FAQPage schema is leaving easy rich results on the table. Flag it and offer to generate.
- **`image` field missing from Article schema** â†’ this is a required field for Article rich results. Google won't show the article card without it.
- **Schema added via GTM** â†’ GTM-injected schema is often not indexed by Google because it renders client-side. Recommend server-side injection.
- **`dateModified` older than `datePublished`** â†’ this is impossible and will fail validation. Flag and fix.
- **Multiple conflicting `@type` on same entity** â†’ e.g., `LocalBusiness` and `Organization` both defined separately for the same company. Should be combined or one should extend the other.
- **Product schema without `offers`** â†’ a Product with no Offer (price, availability, currency) won't earn a product rich result. Flag the missing Offer block.
...
- **Confidence tagging** â€” ðŸŸ¢ verified (test passed) / ðŸŸ¡ medium (valid but untested) / ðŸ”´ assumed (needs verification)

---

## Related Skills

- **seo-audit**: For full technical and content SEO audit. Use seo-audit when the problem spans more than just structured data. NOT for schema-specific work â€” use schema-markup.
