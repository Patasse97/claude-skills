---
title: "Site Architecture & Internal Linking â€” Agent Skill for Marketing"
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/marketing-skill/site-architecture/SKILL.md">Source</a></span>
</div>
...
- Specific SEO targets â€” topics or keyword clusters they want to rank for
...
A URL should tell a user exactly where they are before they click. It also tells search engines about content hierarchy. Get this right once â€” URL changes later require redirects and lose equity.
...
| 4+ levels | `/a/b/c/d/page` | âŒ Avoid â€” dilutes crawl equity, confusing |
...
| `/services/web-design` | `/services/web-design/` (trailing slash â€” pick one and be consistent) |
| `/about` | `/about-us-company-info` (keyword stuffing the URL) |
| Short, human-readable | Long, generated, token-filled |

### Keywords in URLs

Yes â€” include the primary keyword. No â€” don't stuff 4 keywords in.

`/guides/technical-seo-audit` âœ…
`/guides/technical-seo-audit-checklist-how-to-complete-step-by-step` âŒ
...
| Footer nav | Secondary utility links, key service pages | Sitewide links â€” use carefully |
...
- Never use nav labels like "Resources" with no landing page â€” it should be a real, rankable resources page.
- Dropdown menus are fine but crawlers may not engage them deeply â€” critical pages need a clickable parent link.
...
Every breadcrumb segment should be a real, crawlable link â€” not just styled text.
...
```
HUB: /seo/                          â† Pillar page, broad topic
  SPOKE: /seo/technical-seo/        â† Sub-topic
  SPOKE: /seo/on-page-seo/          â† Sub-topic
  SPOKE: /seo/link-building/        â† Sub-topic
  SPOKE: /seo/keyword-research/     â† Sub-topic
    â””â”€ DEEP: /seo/keyword-research/long-tail-keywords/   â† Specific guide
```
...
- Cross-silo links are fine when genuinely relevant â€” just don't build a link for its own sake
...
6. Build the cluster before you build the links â€” if you don't have the content, the links don't help
...
- A page with no internal links is an orphan â€” Google won't prioritize it
...
| Type | Example | Use |
|------|---------|-----|
| Exact match | "cold email templates" | Use sparingly â€” 1-2x per page, looks natural |
| Partial match | "writing effective cold emails" | Primary approach â€” most internal links |
| Branded | "our email guide" | Fine, not the most powerful |
| Generic | "click here", "learn more" | Avoid â€” wastes the signal |
...
1. **In-content links** â€” within the body copy of a relevant page. Most natural, most powerful.
2. **Hub page links** â€” the pillar page linking to all its spokes. High equity because pillar pages are linked from everywhere.
3. **Navigation links** â€” sitewide, consistent, but diluted by their ubiquity.
4. **Footer links** â€” sitewide, but Google gives them less weight than in-content.
5. **Sidebar links** â€” OK but often not in the main content flow.
...
| Navigation that doesn't match user intent | Users leave, rankings drop | Run card-sort tests â€” let users show you their mental model |
| Homepage linking nowhere | Home is highest-equity page â€” use it | Link from home to key hub pages |
...
- **Pages more than 3 clicks from homepage** â†’ flag as crawl equity risk. Any page a user has to click 4+ times to reach needs a structural shortcut.
- **Category/hub page has thin or no content** â†’ hub pages without real content don't rank. Flag and recommend adding a proper pillar page.
- **Internal links using generic anchor text ("click here", "read more")** â†’ wasted signal. Offer to rewrite anchor text patterns.
- **No breadcrumbs on deep pages** â†’ missing upward equity links and BreadcrumbList schema opportunity.
- **Sitemap includes noindex pages** â†’ sitemap should only contain pages you want indexed. Flag and offer to filter.
- **Primary nav links to utility pages (contact, privacy)** â†’ pushing equity to low-value pages. Nav should prioritize money/content pages.
...
- **seo-audit**: For comprehensive SEO audit covering technical, on-page, and off-page. Use seo-audit when architecture is one of several problem areas. NOT for deep structural redesign â€” use site-architecture.
...
- **seo-audit**: For identifying technical issues. NOT for architecture redesign planning â€” use site-architecture for that.
