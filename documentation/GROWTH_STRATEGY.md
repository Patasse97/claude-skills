...
| Agent | Primary Skill | Secondary Skills | Python Tools | Status |
|-------|---------------|------------------|--------------|--------|
| cs-content-creator | content-creator | social-media-analyzer | 8 tools | ðŸ“‹ Planned |
| cs-demand-gen-specialist | marketing-demand-acquisition | - | 4 tools | âœ… Active |
| cs-aso-specialist | app-store-optimization | - | 6 tools | ðŸ“‹ Planned |
| cs-social-media-manager | social-media-analyzer | content-creator | 3 tools | ðŸ“‹ Planned |
| cs-ceo-advisor | ceo-advisor | - | 0 (strategic) | âœ… Active |
| cs-cto-advisor | cto-advisor | - | 0 (strategic) | âœ… Active |
| cs-product-manager | product-manager-toolkit | - | 8 tools | âœ… Active |
| cs-aws-architect | aws-solution-architect | - | 4 tools | ðŸ“‹ Planned |
| cs-ms365-admin | ms365-tenant-manager | - | 5 tools | ðŸ“‹ Planned |
...
**DON'T:**
- âŒ Rename existing Python tools (create new, deprecate old)
- âŒ Change Python tool required parameters
- âŒ Remove workflows from SKILL.md (mark deprecated instead)
- âŒ Change folder structure of existing skills
- âŒ Break relative path references in agents
- âŒ Remove or rename files that agents reference
...
- ðŸ“‹ SEO Optimizer (Q1 2026)
- ðŸ“‹ Social Media Manager (Q1 2026)
...
- ðŸ“‹ Asana Expert (Q2 2026)
- ðŸ“‹ Monday.com Expert (Q2 2026)
...
- ðŸ“‹ Google Cloud Architect (Q2 2026)
- ðŸ“‹ Azure Solution Architect (Q2 2026)
- ðŸ“‹ Mobile Engineer (Q3 2026)

**Engineering - AI/ML/Data (5 â†’ 7):**
- âœ… 5 existing AI/ML/Data skills
- ðŸ“‹ MLOps Engineer (Q2 2026)
- ðŸ“‹ NLP Engineer (Q3 2026)

**RA/QM (12 â†’ 12):** Complete, mature domain

**Business & Growth (3 â†’ 4):**
- âœ… Customer Success Manager
- âœ… Sales Engineer
- âœ… Revenue Operations
- ðŸ“‹ Growth Marketer (Q2 2026)

**Finance (1 â†’ 1):** Complete
...
if [ ! -d "$SKILL_PATH/references" ]; then
  echo "âŒ Missing references/ directory"
  exit 1
fi

if [ ! -d "$SKILL_PATH/assets" ]; then
  echo "âŒ Missing assets/ directory"
  exit 1
fi
...
if [ -d "$SKILL_PATH/scripts" ]; then
  for tool in "$SKILL_PATH/scripts"/*.py; do
    if [ -f "$tool" ]; then
      ...
      if [ $? -ne 0 ]; then
        echo "âŒ Tool $(basename $tool) missing --help flag"
        exit 1
      fi
    fi
  done
...
if grep -r "/Users/" "$SKILL_PATH" > /dev/null; then
  echo "âŒ Hardcoded /Users/ paths found"
  exit 1
fi

if grep -r "C:\\" "$SKILL_PATH" > /dev/null; then
  echo "âŒ Hardcoded C:\\ paths found"
  exit 1
fi
...
if ! grep -q "$NEW_SKILL_NAME" README.md; then
  echo "âŒ README.md not updated with skill"
  exit 1
fi
...
if [ -d "*/scripts" ]; then
  if ! grep -q "$NEW_SKILL_NAME" documentation/PYTHON_TOOLS_AUDIT.md; then
    echo "âŒ PYTHON_TOOLS_AUDIT.md not updated"
    exit 1
  fi
fi
