...
```html
<header role="banner">         <!-- Site header â€” once per page -->
<nav role="navigation">         <!-- Navigation â€” can have multiple with aria-label -->
<main role="main">              <!-- Main content â€” once per page -->
<aside role="complementary">    <!-- Sidebar â€” related but not essential -->
<footer role="contentinfo">     <!-- Site footer â€” once per page -->
<form role="search">            <!-- Search form -->
```

**Semantic HTML equivalents:** `<header>`, `<nav>`, `<main>`, `<aside>`, `<footer>` provide implicit roles â€” no need to double up with explicit `role` attributes.
...
```
Tab          â†’ Move to tab list, then to tab panel
Arrow Left/Right â†’ Switch between tabs
Home         â†’ First tab
End          â†’ Last tab
```
...
```
Arrow Down   â†’ Open list / next option
...
```
Enter/Space  â†’ Activate item
Arrow Down   â†’ Next item
Arrow Up     â†’ Previous item
Arrow Right  â†’ Open submenu
Arrow Left   â†’ Close submenu
Escape       â†’ Close menu
```
...
```
Enter/Space  â†’ Toggle section
Arrow Down   â†’ Next header
Arrow Up     â†’ Previous header
Home         â†’ First header
End          â†’ Last header
```
...
| Redundant ARIA on semantic HTML | `<nav role="navigation">` is redundant | Drop the `role` â€” `<nav>` implies it |
