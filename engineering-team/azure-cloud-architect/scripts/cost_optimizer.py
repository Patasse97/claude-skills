        ...
        for db in dbs:
            ...
            if ru_provisioned > 0 and ru_used / ru_provisioned < 0.2:
                ...
                self.recommendations.append({
                    "service": "Cosmos DB",
                    "type": "Autoscale",
                    "issue": f"Container {db.get('name', '?')} uses {ru_used}/{ru_provisioned} RU/s â€” variable workload",
...
def _format_text(report: Dict[str, Any]) -> str:
    ...
    for rec in report.get("recommendations", []):
        lines.append(f"  [{rec['priority'].upper()}] {rec['service']} â€” {rec['type']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Azure Cost Optimizer â€” analyze Azure resources and recommend cost savings.",
        epilog="Examples:\n"
               "  python cost_optimizer.py --config resources.json\n"
               "  python cost_optimizer.py --config resources.json --json",
