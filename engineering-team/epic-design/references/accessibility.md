...
Every 2.5D website MUST implement ALL of the following. These are not optional enhancements â€” they are legal requirements in many jurisdictions and ethical requirements always.

---

## 1. prefers-reduced-motion (Most Critical)

Parallax and complex animations can trigger vestibular disorders â€” dizziness, nausea, migraines â€” in a significant portion of users. WCAG 2.1 Success Criterion 2.3.3 requires handling this.
...
```javascript
// Also check in JavaScript â€” some GSAP animations don't respect CSS media queries
...
|---|---|
| Scroll parallax depth layers | DISABLE â€” continuous motion triggers vestibular issues |
| Float loops / ambient movement | DISABLE â€” looping motion is a trigger |
| DJI scale-in / perspective zoom | DISABLE â€” fast scale can cause dizziness |
| Particle systems | DISABLE |
| Clip-path reveals (one-shot) | KEEP â€” not continuous, not fast |
| Fade-in on scroll (opacity only) | KEEP â€” safe |
| Word-by-word scroll lighting | KEEP â€” no movement, just colour |
...
if (prefersReduced) {
  ...
  // Slow GSAP to near-freeze (don't fully kill â€” keep structure intact)
...
<main>
  ...
  <section aria-label="Hero â€” Product Introduction">
    ...
    <div class="layer depth-3">
      <img 
        src="product.png" 
        alt="[Descriptive alt text â€” what is the product, what does it look like]"
    ...
    <div class="layer depth-4">
      ...
      <!-- h1 is the page title â€” only one per page -->
...
When using SplitText to fragment text into characters/words, the individual fragments get announced one at a time by screen readers â€” which sounds terrible. Fix this:
...
```css
/* Ensure focus indicators are visible â€” WCAG 2.4.7 */
:focus-visible {
...
/* Safe text over complex backgrounds â€” add text shadow for contrast boost */
.hero-text-on-image {
  color: #ffffff;
...
## 6. Motion-Sensitive Users â€” User Control
...
>
  <span class="motion-toggle-icon">âœ¦</span>
...
## 7. Images â€” Alt Text Guidelines
...
<!-- Standalone icon button â€” needs alt text -->
<button>
