...
The AI inspects, judges, and informs â€” it does NOT auto-remove backgrounds.

---

## Step 1 â€” Run the Inspection Script
...
## Step 2 â€” Judge Whether Background Removal Is Actually Needed
...
- Any image placed at depth-0 â€” it IS the background, keep it

### When unsure â€” ask the role:
> "Does this image need to float freely over other content?"
> Yes â†’ remove bg. No â†’ keep it.

---

## Step 3 â€” Resize to Depth-Appropriate Dimensions
...
## Step 4 â€” Inform the User (Required for Every Asset)
...
>
> **Options:**
> 1. Provide a new PNG with a transparent background â€” best quality, ideal
> 2. Proceed as-is with a CSS workaround (mix-blend-mode) â€” quick but approximate
> 3. Keep the background â€” if this image is meant to be seen with its background
>
> Which do you prefer?

For clean images, confirm them briefly:

> âœ… **[filename]** â€” clean transparent PNG, resized to [X]px, assigned depth-[N] ([role])

Show all of this BEFORE outputting HTML. Wait for the user's response on any âš ï¸ items.

---

## Step 5 â€” CSS Workaround (Only After User Approves)

Apply ONLY if the user explicitly chooses option 2 above:

```css
/* Dark background image on a dark site â€” black pixels become invisible */
.on-dark-bg {
  mix-blend-mode: screen;
}

/* Light background image on a light site â€” white pixels become invisible */
.on-light-bg {
...
Limitations:
- `screen` lightens mid-tones â€” only works well on very dark site backgrounds
- `multiply` darkens mid-tones â€” only works well on very light site backgrounds
- Neither works on complex or gradient backgrounds
- A proper cutout PNG always gives better results

---

## Step 6 â€” CSS Rules for Transparent Images

Whether the image came in clean or had its background resolved, always apply:

```css
/* ALWAYS use drop-shadow â€” it follows the actual pixel shape */
.product-img {
  filter: drop-shadow(0 30px 60px rgba(0, 0, 0, 0.4));
}

/* NEVER use box-shadow on cutout images â€” it creates a rectangle, not a shape shadow */

/* NEVER apply these to transparent/cutout images: */
/*
  border-radius           â†’ clips transparency into a rounded box
  overflow: hidden        â†’ same problem on the parent element
  object-fit: cover       â†’ stretches image to fill a box, destroys the cutout
  background-color        â†’ makes the bounding box visible
*/
```
