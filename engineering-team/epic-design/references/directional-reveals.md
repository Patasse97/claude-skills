...
Elements and sections don't always enter from the bottom. Premium sites use **directional births** â€” sections that drop from the top, iris open from center, peel away like wallpaper, or unfold diagonally. This file covers all 8 directional reveal patterns.
...
```css
/* Starting state â€” section is fully clipped (invisible) */
.top-drop-section {
  /* Section exists in DOM but is invisible */
  clip-path: inset(0 0 100% 0);
  /* 
    inset(top right bottom left):
    - top: 0 â†’ clip starts at top edge
    - bottom: 100% â†’ clips 100% from bottom = nothing visible
  */
}
...
function addTopRetractExit(sectionEl) {
  gsap.to(sectionEl, {
    clipPath: 'inset(100% 0 0% 0)', // now clips from TOP â€” retracts upward
    ease: 'power2.in',
    scrollTrigger: {
      trigger: sectionEl,
      start: 'bottom 20%',
...
**Key insight:** Enter = `inset(0 0 100% 0)` â†’ `inset(0 0 0% 0)` (bottom clips away downward).
Exit = `inset(0)` â†’ `inset(100% 0 0 0)` (top clips away upward = retracts back where it came from).

---

## Pattern 2: Window Pane Iris Open {#iris-open}

An entire section starts as a tiny centered rectangle â€” like a keyhole or portal â€” and expands outward to fill the viewport. Creates a cinematic "opening shot" feeling.
...
**Variation â€” horizontal bar open (blinds effect):**
```javascript
...
.curtain-panel {
  position: absolute;
  inset: 0;
  /* Stack panels â€” panel 1 on top, panel N on bottom */
}
...
function initCurtainRollUp(containerEl) {
  ...
  panels.forEach((panel, i) => {
    ...
    // Each panel rolls up â€” clip from bottom rises to top
    tl.to(panel, {
...
The section's edge is not a hard straight line â€” it morphs between shapes (rectangle â†’ wave â†’ diagonal â†’ organic curve) as the user scrolls. Makes sections feel alive and fluid.
...
function initSVGMorphBorder() {
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // Morph between straight â†’ wave â†’ diagonal as scroll progresses
      if (p < 0.5) {
        // Interpolate straight â†’ wave
...
Content is revealed by a diagonal sweep across the screen â€” from top-left corner to bottom-right (or any corner combination). Feels cinematic and directional.
...
When a grid or set of cards appears, each item enters from a different edge/direction â€” creating a dynamic assembly effect instead of uniform fade-ups.
...
```
Section 1 â†’ Section 2: Window pane iris (section 2 peeks through a keyhole)
Section 2 â†’ Section 3: Top-down clip birth (section 3 drops from top)
Section 3 â†’ Section 4: Diagonal wipe (section 4 sweeps in from corner)
Section 4 â†’ Section 5: Circle iris (section 5 opens from center)
Section 5 â†’ Section 6: Curtain panel roll-up (exposes multiple layers)
```
