...
These are the most premium techniques â€” effects where elements **persist, travel, or transition between sections**, creating a seamless narrative thread across the entire page.
...
This is THE signature technique for product brands. A product image (juice bottle, phone, sneaker) starts inside the hero section. As you scroll, it appears to "rise up" through the section boundary and hover between two differently-colored sections â€” partially owned by neither. Then as you continue scrolling, it gracefully descends back in.
...
.inter-section-product-wrapper {
  ...
  z-index: 100;
  pointer-events: none;
  height: 0; /* no height â€” just a position anchor */
}
...
function initFloatingProduct() {
  ...
  // Phase 2: Product fully "between" sections â€” peak visibility
  tl.to(productImg, {
    y: '-5vh',
    scale: 1.1,
    duration: 0.3,
...
<section class="hero-section" style="background: #0a0014; min-height: 100vh; position: relative; z-index: 1;">
  ...
  <!-- NO product image here â€” it's in the inter-section wrapper -->
...
<!-- THE FLOATING PRODUCT â€” outside both sections, between them -->
<div class="inter-section-product-wrapper">
  <div class="inter-section-product">
    <img 
      src="product.png" 
      alt="Product Name â€” floating between hero and features"
      class="float-loop"
    />
  </div>
</div>
...
The product image starts completely hidden below the section's bottom border â€” clipped out of existence. As the user scrolls into the section boundary, the product "grows up" through the border like a plant emerging from soil. This is distinct from the floating product â€” here, the section itself is the stage.
...
.birth-product {
  position: absolute;
  bottom: -20%;  /* starts 20% below the section â€” invisible */
  left: 50%;
...
function initClipPathBirth(sectionEl, productEl) {
  ...
  // Continue scroll â†’ product rises further and becomes full height
...
Made famous by DJI drone product pages. A section starts with a small, contained image. As the user scrolls, the image scales up to fill the entire viewport â€” THEN the section unpins and the next content reveals. Creates a "zoom into the world" feeling.
...
The most advanced technique. A product element travels along a smooth, curved Bezier path across the page as the user scrolls â€” arcing through space like it's floating or being thrown, rather than just translating in a straight line.
...
The section below is revealed by the section above peeling away â€” like turning a page. Uses `sticky: bottom: 0` so the lower section sticks to the screen bottom while the upper section scrolls away.
...
| Dark â†’ light section transition | Floating Product (section backgrounds change beneath) |
