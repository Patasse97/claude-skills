...
```
âœ… SAFE (GPU composited):    transform, opacity, filter, clip-path, will-change
âŒ AVOID (triggers layout):  width, height, top, left, right, bottom, margin, padding,
                              font-size, border-width, background-size (avoid)
```

Animating layout properties causes the browser to recalculate the entire page layout on every frame â€” this is called "layout thrash" and causes jank.
...
// passive: true is CRITICAL â€” tells browser scroll handler won't preventDefault
...
* { will-change: transform; } /* WRONG â€” massive GPU memory usage */
...
**Note:** Don't apply to the first section â€” it causes a flash of invisible content.
...
|-------------|---------------------|---------------|----------------|
| Depth 0     | Background          | 150KB         | 1920Ã—1080      |
| Depth 1     | Glow layer          | 60KB          | 1000Ã—1000      |
| Depth 2     | Decorations         | 50KB          | 400Ã—400        |
| Depth 3     | Main product/hero   | 120KB         | 1200Ã—1200      |
| Depth 4     | UI components       | 40KB          | 800Ã—800        |
| Depth 5     | Particles           | 10KB          | 128Ã—128        |
...
1. **Layers panel**: Check `chrome://settings` â†’ DevTools â†’ "Show Composited Layer Borders" â€” should not show excessive layer count (target: under 20 promoted layers)
2. **Performance tab**: Record scroll at 60fps. Look for long frames (>16ms)
3. **Memory tab**: Heap snapshot â€” should not grow during scroll (no leaks)
4. **Coverage tab**: Check unused CSS/JS â€” strip unused animation classes
...
// GOOD: Use scrub â€” GSAP manages timing internally
gsap.to(element, {
  y: 200,
  ease: 'none',
  scrollTrigger: {
