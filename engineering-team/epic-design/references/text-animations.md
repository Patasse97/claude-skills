...
function initSplitConverge(titleEl) {
  ...
  // Phase 1 â€” ENTER (0% â†’ 25%): Words converge from sides
  tl.fromTo(leftWords,
    { x: '-120vw', opacity: 0 },
  ...
  // Phase 2 â€” HOLD (25% â†’ 70%): Nothing â€” words are readable, section pinned
  // (empty duration keeps the scrub paused here)
  .to({}, { duration: 0.45 }, 0.25)

  // Phase 3 â€” EXIT (70% â†’ 100%): Words diverge back out
  .to(leftWords,
...
.curtain-text .line-mask {
  overflow: hidden;
  line-height: 1.2;
  /* The mask â€” content starts below and slides up into view */
}
...
Letters rotate in on a 3D cylinder axis â€” like a slot machine or odometer rolling into place. Premium, memorable.
...
Element automatically animates in when entering the viewport AND animates out when leaving â€” zero JavaScript needed.
...
.offset-title {
  position: relative;
  /* Don't center â€” let offset do the work */
}
.offset-title .line-1 {
  /* Top-left */
  display: block;
...
.offset-title .line-2 {
  /* Lower-right â€” drops down and shifts right */
  display: block;
  text-align: right;
  padding-right: 5%;
  margin-top: 0.4em;
...
Infinite scrolling text. Speed scales with scroll velocity â€” fast scroll = fast marquee. Slow scroll = slow/paused.
...
```javascript
// Parallax on the bleed title â€” moves at slightly different rate
...
- `0.08â€“0.12` â†’ barely-there atmosphere
- `0.15â€“0.22` â†’ readable on inspection, still subtle
- `0.25â€“0.35` â†’ prominently visible â€” only if it IS the visual focus

Rules:
1. Always `aria-hidden="true"` â€” never the real heading
2. A real `<h1>` must exist elsewhere for SEO/screen readers
3. Only works on dark backgrounds â€” thin strokes vanish on light ones
4. Maximum 2 lines â€” 3+ becomes noise
5. Best with ultra-heavy weights (800â€“900) and tight letter-spacing
