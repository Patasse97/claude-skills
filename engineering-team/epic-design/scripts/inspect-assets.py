...
The script NEVER modifies images â€” inspect only.
"""
...
def analyse_image(path):
    ...
    if img.mode == "RGBA":
        ...
        if alpha_min == 0:
            ...
            result["notes"].append("Real alpha channel with transparent pixels â€” clean cutout")
            result["likely_needs_removal"] = False
            return result
        else:
            result["notes"].append("RGBA mode but alpha is fully opaque â€” background was never removed")
    ...
    if variance > 80:
        ...
        result["notes"].append(
            "Background varies significantly across edges â€” likely a scene, "
        ...
        result["notes"].append(
            f"Solid dark background detected â€” average edge brightness: {avg_brightness:.0f}/255"
        )
        ...
        result["notes"].append(
            f"Solid light background detected â€” average edge brightness: {avg_brightness:.0f}/255"
        )
        result["likely_needs_removal"] = True
        result["notes"].append(
        ...
        result["notes"].append(
            f"Solid mid-tone background detected â€” avg colour: RGB{result['bg_colour']}"
        )
        result["likely_needs_removal"] = None  # ambiguous â€” let AI judge
        result["notes"].append(
            "JUDGMENT: Ambiguous â€” could be a branded background (keep) or a "
    ...
    if result["format"] in ("JPEG", "JPG"):
        result["notes"].append(
            "JPEG format â€” cannot store transparency. "
    ...
    if w > 2000 or h > 2000:
        result["notes"].append(
            f"Large image ({w}x{h}px) â€” resize before embedding. "
...
def print_report(results):
    ...
    for r in results:
        ...
        if r["likely_needs_removal"] is True:
            ...
            print("    Removal: Ambiguous â€” AI must judge from context")
