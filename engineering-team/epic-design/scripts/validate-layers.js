...
if (!filePath) {
  console.error('\nâŒ  Usage: node validate-layers.js path/to/index.html\n');
  process.exit(1);
}
...
function check(label, condition, suggestion) {
  if (condition) {
    passed++;
    results.push({ status: 'âœ…', label });
  } else {
    failed++;
    results.push({ status: 'âŒ', label, suggestion });
  }
}

function warn(label, condition, suggestion) {
  if (!condition) {
    results.push({ status: 'âš ï¸ ', label, suggestion });
  }
}
...
console.log('\nðŸ“‹  2.5D Layer Validator Report');
...
if (failed === 0) {
  console.log('\nðŸŽ‰  All checks passed! Your 2.5D site is ready.\n');
} else {
  console.log(`\nðŸ”§  Fix the ${failed} issue(s) above before shipping.\n`);
  process.exit(1);
}
