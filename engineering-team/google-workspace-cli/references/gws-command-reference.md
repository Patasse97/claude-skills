...
|----------|-------------|---------|
| `GWS_CLIENT_ID` | OAuth client ID | â€” |
| `GWS_CLIENT_SECRET` | OAuth client secret | â€” |
| `GWS_TOKEN_PATH` | Token storage location | `~/.config/gws/token.json` |
| `GWS_SERVICE_ACCOUNT_KEY` | Service account JSON key path | â€” |
| `GWS_DELEGATED_USER` | User to impersonate (service accounts) | â€” |
