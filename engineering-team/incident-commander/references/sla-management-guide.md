...
```
  SLA (99.9%)     â† Contract with customers, financial penalties
    â†‘ backs
  SLO (99.95%)    â† Internal target, triggers error budget policy
    â†‘ targets
  SLI (measured)  â† Raw metric: actual uptime = 99.97% this month
```
...
Shortfall: 0.228 percentage points â†’ Major Breach
```
...
Error Budget Consumed: 165 / 223.2 = 73.9% â†’ Yellow threshold, feature freeze recommended
```
