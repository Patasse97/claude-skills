...
try {
    ...
    Write-Host "âœ“ Conditional Access policy created: $($newPolicy.DisplayName)" -ForegroundColor Green
...
Write-Host "  Users without MFA: $usersWithoutMFA $(if($usersWithoutMFA -gt 0){'âš ï¸'}else{'âœ“'})"
Write-Host "  Inactive Users (90+ days): $($inactiveUsers.Count) $(if($inactiveUsers.Count -gt 0){'âš ï¸'}else{'âœ“'})"
...
if (-not $licenseSkuId) {{
    Write-Host "âœ— License SKU not found: $targetSku" -ForegroundColor Red
    exit
}}
...
if (-not $users) {{
    Write-Host "âœ— No users found in CSV file" -ForegroundColor Red
    exit
}}
...
foreach ($user in $users) {{
    ...
    try {{
        ...
        if ($currentLicenses.SkuId -contains $licenseSkuId) {{
            Write-Host "  âŠ˜ $userEmail - Already has license" -ForegroundColor Yellow
        ...
        Write-Host "  âœ“ $userEmail - License assigned successfully" -ForegroundColor Green
        ...
        Write-Host "  âœ— $userEmail - Error: $_" -ForegroundColor Red
        $errorCount++
        $results += [PSCustomObject]@{{
            UserPrincipalName = $userEmail
            Status = "Failed"
