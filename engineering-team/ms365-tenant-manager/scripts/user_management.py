...
foreach ($user in $users) {
    try {
        ...
        Write-Host "  âœ“ License assigned: $($user.LicenseSku)" -ForegroundColor Green
...
try {{
    ...
    Write-Host "âœ“ Account disabled - user cannot sign in" -ForegroundColor Green

    # Step 3: Revoke all active sessions
    Revoke-MgUserSignInSession -UserId $user.Id
    Write-Host "âœ“ All active sessions revoked" -ForegroundColor Green
    ...
    Write-Host "âœ“ Removed from all groups" -ForegroundColor Green
    ...
    Write-Host "âœ“ All mobile devices removed" -ForegroundColor Green

    # Step 6: Convert mailbox to shared (preserves emails, removes license requirement)
    Set-Mailbox -Identity $userEmail -Type Shared
    Write-Host "âœ“ Mailbox converted to shared mailbox" -ForegroundColor Green
    ...
    # Write-Host "âœ“ Email forwarding configured" -ForegroundColor Green
...
"@
    ...
    Write-Host "âœ“ Auto-reply configured" -ForegroundColor Green
    ...
    if ($licenses) {{
        ...
        Write-Host "âœ“ Licenses removed" -ForegroundColor Green
    }}

    # Step 10: Hide from GAL (Global Address List)
    Set-Mailbox -Identity $userEmail -HiddenFromAddressListsEnabled $true
    Write-Host "âœ“ Hidden from Global Address List" -ForegroundColor Green
    ...
    Write-Host "`nâœ“ Offboarding completed successfully!" -ForegroundColor Green
    ...
    Write-Host "âœ— Error during offboarding: $_" -ForegroundColor Red
}}
