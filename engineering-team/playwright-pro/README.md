...
Generate tests, fix flaky failures, migrate from Cypress/Selenium, sync with TestRail, run on BrowserStack â€” all from your AI agent.
...
|---|---|
| `/pw:init` | Set up Playwright in your project â€” detects framework, generates config, CI, first test |
...
| `/pw:testrail` | Sync with TestRail â€” read cases, push results, create runs |
...
# Tests are auto-validated by hooks â€” no extra steps
```
...
- **test-architect** â€” Plans test strategy for complex applications
- **test-debugger** â€” Diagnoses flaky tests using a systematic taxonomy
- **migration-planner** â€” Creates file-by-file migration plans from Cypress/Selenium
...
- **TestRail** â€” Read test cases, create runs, push pass/fail results
- **BrowserStack** â€” Trigger cross-browser runs, pull session reports with video/screenshots
...
| Agent | How |
|---|---|
| **Claude Code** | Full plugin â€” slash commands, MCP tools, hooks, agents |
