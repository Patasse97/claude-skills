...
1. **Shared resources first**: custom commands â†’ fixtures, page objects â†’ helpers
...
1. Convert custom commands â†’ fixtures.ts
2. Copy fixtures â†’ test-data/
3. Convert page objects (API changes only)

### Phase 2: Simple Tests (quick wins)
4. login.cy.ts â†’ auth/login.spec.ts (5 tests, ~15 min)
5. about.cy.ts â†’ static/about.spec.ts (2 tests, ~5 min)

### Phase 3: Complex Tests
6. checkout.cy.ts â†’ checkout/checkout.spec.ts (12 tests, ~45 min)
7. search.cy.ts â†’ search/search.spec.ts (8 tests, ~30 min)
```
