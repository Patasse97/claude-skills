...
```typescript
// BAD â€” checks once, no retry
const text = await page.textContent('.msg');
expect(text).toBe('Done');

// GOOD â€” retries until timeout
...
// GOOD â€” uses baseURL from config
...
```typescript
// BAD â€” test B depends on test A
let userId: string;
...
```typescript
// BAD â€” waits for all network activity to stop
await page.goto('/dashboard', { waitUntil: 'networkidle' });

// GOOD â€” wait for specific content
await page.goto('/dashboard');
...
```typescript
// BAD â€” click navigates but we don't wait
await page.getByRole('link', { name: 'Settings' }).click();
await expect(page.getByRole('heading')).toHaveText('Settings');

// GOOD â€” wait for URL change
await page.getByRole('link', { name: 'Settings' }).click();
...
```typescript
// BAD â€” tests CSS class (implementation detail)
await expect(page.locator('.btn')).toHaveClass('btn-primary active');

// GOOD â€” tests what the user sees
