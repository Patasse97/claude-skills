...
|---|---|
| `new Builder().forBrowser('chrome').build()` | Handled by config â€” no driver setup |
| `driver.quit()` | Automatic â€” Playwright manages browser lifecycle |
...
| `By.xpath('//div')` | `page.locator('xpath=//div')` | Avoid â€” use role-based |
| `By.name('x')` | `page.locator('[name=x]')` | `page.getByLabel(...)` |
| `By.linkText('x')` | `page.getByRole('link', { name: 'x' })` | âœ… Best practice |
| `By.partialLinkText('x')` | `page.getByRole('link', { name: /x/ })` | âœ… Best practice |
| `By.tagName('button')` | `page.getByRole('button')` | âœ… Best practice |
...
| `Thread.sleep(3000)` | âŒ Never use | Use assertions instead |
...
| `driver.switchTo().defaultContent()` | Not needed â€” use `page` directly |
...
1. **No implicit waits** â€” Playwright auto-waits for actionability
2. **No driver management** â€” Playwright handles browser lifecycle
3. **Built-in assertions** â€” `expect(locator)` with auto-retry
4. **Parallel by default** â€” tests run in parallel, must be isolated
5. **Traces instead of screenshots** â€” richer debugging artifacts
