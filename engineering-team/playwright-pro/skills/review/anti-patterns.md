...
- No page objects for complex pages â†’ create them
- Inline data â†’ use factories or fixtures
- Missing a11y assertions â†’ add `toHaveAttribute('role', ...)`
- No visual regression â†’ add `toHaveScreenshot()` for key pages
- Not checking console errors â†’ add `page.on('console', ...)`
- Using `networkidle` â†’ use specific assertions instead
- No `test.describe()` â†’ group related tests
