...
test.describe('Color Contrast', () => {
  ...
  // Happy path: body text contrast ratio â‰¥ 4.5:1
  test('body text meets WCAG AA contrast ratio', async ({ page }) => {
    const ratio = await page.evaluate(() => {
      ...
      // Simplified check â€” use axe for full verification
      return style.color !== 'rgba(0, 0, 0, 0)' ? style.color : null;
    });
    expect(ratio).toBeTruthy();
  });

  // Happy path: large text contrast ratio â‰¥ 3:1
  ...
  test('full page passes WCAG 2.1 AA axe scan', async ({ page }) => {
    ...
    if (results.violations.length > 0) {
      const messages = results.violations.map(v =>
        `${v.id}: ${v.description} â€” ${v.nodes.map(n => n.target).join(', ')}`
      ).join('\n');
...
| Variant | Description |
|---------|-------------|
| Contrast violations | axe color-contrast rule â†’ no violations |
| Body text contrast | Text color non-transparent |
| Heading contrast | axe include h1-h6 â†’ no violations |
| Focus indicator | outline-width â‰¥ 2px and non-transparent |
