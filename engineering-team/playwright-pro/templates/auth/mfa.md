...
test.describe('MFA', () => {
  ...
  test('accepts backup code', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /verify/i }).click();
    await expect(page).toHaveURL('{{baseUrl}}/dashboard');
    // Backup code consumed â€” warning shown
...
| Variant | Description |
|---------|-------------|
| Valid TOTP | Correct time-based code â†’ dashboard |
| Backup code | Single-use backup code accepted; warning shown |
| Invalid code | Wrong code â†’ alert, stays on MFA page |
| Expired code | Clock-advanced token rejected |
| MFA enrollment | QR shown â†’ token verified â†’ backup codes displayed |
