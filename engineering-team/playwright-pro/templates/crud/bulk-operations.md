...
| Variant | Description |
|---------|-------------|
| Select all + delete | All rows selected â†’ confirmed delete â†’ empty list |
| Partial select + update | N rows selected â†’ status updated â†’ success |
| Toolbar visibility | Appears on select, hides on deselect |
| Deselect all | Select all â†’ uncheck â†’ toolbar gone |
