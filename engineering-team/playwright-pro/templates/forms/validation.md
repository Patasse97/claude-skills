...
| Variant | Description |
|---------|-------------|
| Error cleared | Valid input â†’ errors removed on next submit |
| Required fields | Empty submit â†’ at least one required error |
| Email format | Blur with bad email â†’ inline error |
| Phone format | Invalid phone â†’ inline error |
| Password length | Too short â†’ character count error |
| Password match | Mismatch â†’ confirmation error |
| Blur validation | Error shown on blur, not just submit |
| aria-describedby | Error programmatically linked to field |
| Max length | Exceeded length â†’ error shown |
