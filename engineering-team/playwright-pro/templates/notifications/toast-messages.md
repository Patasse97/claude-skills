...
test.describe('Toast Messages', () => {
  ...
  test('dismisses toast via close button', async ({ page }) => {
    ...
    const toast = page.getByRole('alert').filter({ hasText: /saved/i });
    await expect(toast).toBeVisible();
    await toast.getByRole('button', { name: /close|dismiss|Ã—/i }).click();
    await expect(toast).toBeHidden();
  });
...
test.describe('Toast Messages', () => {
  ...
  test('dismisses toast via close button', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /save/i }).click();
    const toast = page.getByRole('alert').filter({ hasText: /saved/i });
    await toast.getByRole('button', { name: /close|Ã—/i }).click();
    await expect(toast).toBeHidden();
  });
});
```
...
| Variant | Description |
|---------|-------------|
| Success toast | Save â†’ green/success alert visible |
| Error toast | 500 â†’ red/error alert visible |
| Warning toast | Trigger action â†’ warning alert |
