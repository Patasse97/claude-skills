...
test.describe('Welcome Tour', () => {
  ...
  // Edge case: close button (Ã—) dismisses tour
  test('Ã— button dismisses tour', async ({ page }) => {
    await page.goto('{{baseUrl}}/dashboard');
    await page.getByRole('dialog', { name: /welcome|tour/i })
      .getByRole('button', { name: /close|Ã—/i }).click();
    await expect(page.getByRole('dialog', { name: /welcome|tour/i })).toBeHidden();
  });
});
```
...
| Full completion | All steps advanced â†’ tour dismissed |
