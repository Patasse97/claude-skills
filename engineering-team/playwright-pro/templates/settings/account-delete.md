...
| Variant | Description |
|---------|-------------|
| Dialog opens | Delete button â†’ confirmation with warning |
| Cancel | Dialog closed, account preserved |
| Type-to-confirm | Button enabled only with correct email |
| Successful delete | Account deleted â†’ /login |
| Wrong email | Input mismatch â†’ button stays disabled |
| Server error | Deletion fails â†’ error alert |
