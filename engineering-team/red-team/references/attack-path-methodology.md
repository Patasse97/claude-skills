...
| Crown jewel node | Target asset â€” defines engagement success | Domain Controller, S3 Data Lake |
...
```
effort_score = detection_risk Ã— (prerequisite_count + 1)
```

Where:
- `detection_risk` is 0.0â€“1.0 (0 = trivial to execute, 1 = will be detected with high probability)
...
```
choke_point_score = (paths_through_node / total_paths_to_all_crown_jewels) Ã— detection_risk
```

Techniques with a high choke point score have high defensive leverage â€” a detection rule for that technique covers the most attack paths.

### Common Choke Points by Environment

**Active Directory Domain:**
- T1003 (Credential Access) â€” required for Pass-the-Hash and most lateral movement
- T1558 (Kerberos Tickets) â€” Kerberoasting provides service account credentials for privilege escalation

**AWS Cloud:**
- iam:PassRole â€” required for most cloud privilege escalation paths
- T1078.004 (Valid Cloud Accounts) â€” credential compromise required for all cloud attack paths

**Hybrid Environment:**
- T1078.002 (Domain Accounts) â€” once domain credentials are obtained, both on-prem and cloud paths open
- T1021.001 (Remote Desktop Protocol) â€” primary lateral movement mechanism in Windows environments
...
|----------|--------|--------|----------|
| High Priority | Low | High | Test first â€” easiest path to critical asset |
| Medium Priority | Low | Low | Test after high priority |
| Medium Priority | High | High | Test â€” complex but high-value if successful |
| Low Priority | High | Low | Test last â€” costly and low-value |

**Effort** is the path's total effort score (lower = easier).
**Impact** is the crown jewel value (defined in RoE â€” Domain Controller = highest, individual workstation = lowest).
...
| credentialed | All techniques â€” full kill-chain available | None (assumes valid credentials = highest starting position) |
