...
# AWS EC2 Metadata (IMDSv2 â€” requires token header)
...
// Modified header â€” set algorithm to none
{"alg": "none", "typ": "JWT"}
...
```
# Read
GET /api/users/{B_id}/profile     â†’ Should be 403
GET /api/orders/{B_orderId}       â†’ Should be 403

# Write
PUT /api/users/{B_id}/settings    â†’ Should be 403
PATCH /api/orders/{B_orderId}     â†’ Should be 403

# Delete
DELETE /api/comments/{B_commentId} â†’ Should be 403
```

**Step 4: ID manipulation**
```
# Sequential IDs â€” increment/decrement
/api/users/100 â†’ /api/users/101

# UUID prediction â€” not practical, but test for leaked UUIDs
# Check if UUIDs appear in other responses

# Encoded IDs â€” decode and modify
/api/users/MTAw â†’ base64 decode = "100" â†’ encode "101" = MTAx

# Hash-based IDs â€” check for predictable hashing
/api/users/md5(email) â†’ compute md5 of known emails
```
...
POST   /api/admin/users                â†’ 403
DELETE /api/admin/users/123            â†’ 403
PUT    /api/admin/settings             â†’ 403
GET    /api/admin/reports              â†’ 403
POST   /api/admin/impersonate/user123  â†’ 403
...
// Mass assignment attempt â€” add privileged fields:
PUT /api/users/profile
{
  "name": "Normal User",
  "email": "user@test.com",
...
```
# IP rotation â€” test if rate limiting is per-IP only
X-Forwarded-For: 1.2.3.4
X-Real-IP: 1.2.3.4
X-Originating-IP: 1.2.3.4

# Case variation â€” test if endpoints are case-sensitive
/api/login
/API/LOGIN
/Api/Login
