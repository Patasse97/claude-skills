...
```
Image â†’ Backbone â†’ Feature Map
                      |
                      +â†’ RPN â†’ Proposals
                      |           |
                      +â†’ RoI Align â† +
...
- Anchor boxes at each position (3 scales Ã— 3 ratios = 9)
...
```
Stage 1 (IoU 0.5) â†’ Stage 2 (IoU 0.6) â†’ Stage 3 (IoU 0.7)
```
...
Where:
- Î³ (focusing parameter) = 2 typically
- Î± (class weight) = 0.25 for background
...
```
RoI Features â†’ FC Layers â†’ Class + BBox
      |
      +â†’ Conv Layers â†’ Mask (28Ã—28 per class)
```
...
- 14Ã—14 or 28Ã—28 mask resolution
...
3. Linear combination: mask = Î£(coefficients Ã— prototypes)
...
```
Image â†’ Backbone â†’ ASPP â†’ Decoder â†’ Segmentation
              â†˜    â†—
          Low-level features
```
...
```
Image â†’ Patch Embedding â†’ [CLS] + Position Embedding
                              â†“
...
**Key details:**
- Patch size: 16Ã—16 or 14Ã—14 typically
...
1. **Shifted Window Attention**
   ...
   - O(n) complexity vs O(nÂ²) for global attention
...
```
Stage 1: 56Ã—56, 96-dim   â†’ Patch Merge
Stage 2: 28Ã—28, 192-dim  â†’ Patch Merge
Stage 3: 14Ã—14, 384-dim  â†’ Patch Merge
Stage 4: 7Ã—7, 768-dim
```
...
P5 â† C5 (1/32)
 â†“
P4 â† C4 + Upsample(P5) (1/16)
 â†“
P3 â† C3 + Upsample(P4) (1/8)
 â†“
P2 â† C2 + Upsample(P3) (1/4)
```
...
```
FPN top-down â†’ Bottom-up augmentation
P2 â†’ N2 â†˜
P3 â†’ N3 â†’ N3 â†˜
P4 â†’ N4 â†’ N4 â†’ N4 â†˜
P5 â†’ N5 â†’ N5 â†’ N5 â†’ N5
```
