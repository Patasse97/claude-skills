#!/usr/bin/env python3
"""
threat_signal_analyzer.py â€” Threat Signal Analysis: Hunt, IOC Sweep, Anomaly Detection

Supports three analysis modes:
  hunt    â€” Score and prioritize a threat hunting hypothesis
  ioc     â€” Process IOC list and emit sweep targets with freshness check
  anomaly â€” Z-score behavioral anomaly detection against a baseline
...
def hunt_mode(args):
    ...
    # Keyword matching â€” multi-word keywords must be checked before single-word
    matched_keywords = []
    seen_keywords = set()
...
def ioc_mode(args):
    ...
    if ioc_date_str:
        try:
            ...
            pass  # invalid date format â€” skip freshness check
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Threat Signal Analyzer â€” Hunt hypothesis scoring, IOC sweep planning, "
    ...
    if args.mode == "hunt":
        ...
        if args.output_json:
            ...
            print(f"Freshness Warn  : {'YES â€” IOCs may be stale' if result['freshness_warning'] else 'No'}")
