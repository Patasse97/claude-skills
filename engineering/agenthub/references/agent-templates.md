...
4. If improved over your previous best â†’ git add . && git commit -m "improvement: {description}"
5. If NOT improved â†’ git checkout -- .
...
- Commit early â€” each improvement is a separate commit
...
- Agent 1: Caching â€” add memoization, HTTP caching headers, query result caching
- Agent 2: Algorithm optimization â€” reduce complexity, better data structures, eliminate redundant work
- Agent 3: I/O batching â€” batch database queries, parallel I/O, connection pooling

**Cross-domain example** (3 agents writing landing page copy):
- Agent 1: Benefit-led â€” open with the top 3 user benefits, feature details below
- Agent 2: Social proof â€” lead with testimonials and case study stats, then features
- Agent 3: Urgency/scarcity â€” limited-time offer framing, countdown CTA, FOMO triggers
...
2. Make the change â€” keep each change small and focused
3. Run the test suite: {eval_cmd}
4. If tests pass â†’ git add . && git commit -m "refactor: {description}"
5. If tests fail â†’ git checkout -- . and try a different approach
6. Post progress update to .agenthub/board/progress/agent-{i}-iter-{n}.md
...
- Preserve public API contracts â€” no breaking changes
- Prefer smaller, well-tested changes over large rewrites
```

**Strategy assignment:** Example strategies for 3 refactoring agents:
- Agent 1: Extract and simplify â€” break large functions into smaller ones, reduce nesting
- Agent 2: Type safety â€” add type annotations, replace Any types, fix type errors
- Agent 3: DRY â€” eliminate duplication, extract shared utilities, consolidate patterns

**Cross-domain example** (restructuring a research report):
- Agent 1: Executive summary first â€” lead with conclusions, supporting data below
- Agent 2: Narrative flow â€” problem â†’ analysis â†’ findings â†’ recommendations arc
- Agent 3: Visual-first â€” diagrams and data tables up front, prose as annotation
...
5. If coverage increased â†’ git add . && git commit -m "test: {description}"
6. If coverage unchanged or tests fail â†’ git checkout -- . and target a different path
...
Constraints:
- Do NOT access other agents' work or results
- Tests must be meaningful â€” no trivially passing assertions
...
- Agent 1: Happy path coverage â€” cover main use cases and expected inputs
- Agent 2: Edge cases â€” boundary values, empty inputs, error conditions
- Agent 3: Integration tests â€” test module interactions, API endpoints, data flows
...
Follow this process:
1. Reproduce the bug â€” run the verification command to confirm it fails
2. Diagnose the root cause using your approach: {strategy}
3. Implement a fix â€” make the minimal change needed
4. Run the verification command: {eval_cmd}
5. If the bug is fixed AND no regressions â†’ git add . && git commit -m "fix: {description}"
6. If NOT fixed â†’ git checkout -- . and try a different angle
...
Constraints:
- Do NOT access other agents' work or results
- Minimal changes only â€” fix the bug, don't refactor surrounding code
...
- Agent 1: Top-down â€” trace from the error message/stack trace back to root cause
- Agent 2: Bottom-up â€” examine recent changes, bisect commits, find the introducing change
- Agent 3: Isolation â€” write a minimal reproduction, narrow down the failing component
...
Strategy assignment is automatic: the coordinator generates N different strategies appropriate to the template and task, assigning one per agent. The coordinator should choose strategies that are **diverse** â€” overlapping strategies waste agents.
