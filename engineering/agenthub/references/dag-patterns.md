...
The **frontier** is the set of commits (branch tips) that have no children. These are the "leaves" of the DAG â€” the latest state of each agent's work.
...
- `hub/*` â€” all AgentHub work
- `hub/{session}/*` â€” all work for one session
- `hub/{session}/agent-{N}/*` â€” all attempts by one agent
...
1. **Never rebase agent branches** â€” rewrites history, breaks DAG
2. **Never force-push** â€” could overwrite other agents' work
3. **Never delete commits** â€” only delete branch refs (commits preserved via tags)
4. **Never amend** agent commits â€” append-only history
5. **Board is append-only** â€” new posts only, no edits
