...
def print_dashboard(root):
    ...
    for domain_dir in sorted(root.iterdir()):
        ...
        for exp_dir in sorted(domain_dir.iterdir()):
            ...
            best_str = f"{stats['best']:.4f}" if stats["best"] is not None else "â€”"
            pct_str = f"{stats['pct_change']:+.1f}%" if stats["pct_change"] is not None else "â€”"
    ...
    print(f"\n{'â”€' * 90}")
    print(f"  autoresearch â€” Dashboard")
    print(f"{'â”€' * 90}")
...
def export_experiment_markdown(experiment_dir, experiment_path):
    ...
    lines.append(f"**Experiments:** {stats['total']} total â€” {stats['keeps']} kept, {stats['discards']} discarded, {stats['crashes']} crashed\n")

    if stats["baseline"] is not None and stats["best"] is not None:
        pct = f" ({stats['pct_change']:+.1f}%)" if stats["pct_change"] is not None else ""
        lines.append(f"**Progress:** `{stats['baseline']:.6f}` â†’ `{stats['best']:.6f}`{pct}\n")
...
def export_dashboard_markdown(root, domain_filter=None):
    ...
    for domain_dir in sorted(root.iterdir()):
        ...
        for exp_dir in sorted(domain_dir.iterdir()):
            ...
            direction = config.get("metric_direction", "lower")
            stats = compute_stats(results, direction)
            best = f"`{stats['best']:.4f}`" if stats["best"] is not None else "â€”"
            pct = f"{stats['pct_change']:+.1f}%" if stats["pct_change"] is not None else "â€”"
