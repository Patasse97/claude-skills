...
Executes a single experiment iteration. The AI agent is the loop â€”
...
def run_evaluation(project_root, eval_cmd, time_budget_minutes, log_file):
    ...
    Note: shell=True is intentional here â€” eval_cmd is user-provided and
...
def run_single(project_root, experiment_dir, config, exp_num, dry_run=False, description=None):
    ...
    # Timeout
    if ret_code == -1:
        print(f"  TIMEOUT after {elapsed:.0f}s â€” discarding")
