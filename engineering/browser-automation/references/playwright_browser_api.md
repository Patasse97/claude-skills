...
This reference covers Playwright's Python async API for browser automation tasks â€” NOT testing. For test-specific APIs (assertions, fixtures, test runners), see playwright-pro.
...
async with async_playwright() as p:
    ...
    # WebKit (Safari engine â€” useful for Apple-specific sites)
...
**`wait_until` options:**
- `"load"` â€” wait for the `load` event (all resources loaded)
- `"domcontentloaded"` â€” DOM is ready, images/styles may still load
- `"networkidle"` â€” no network requests for 500ms (best for SPAs)
- `"commit"` â€” response received, before any rendering
...
# Fixed wait (use sparingly â€” prefer the methods above)
...
# Locator API (recommended â€” auto-waits, re-queries on each action)
...
- `query_selector` â€” returns an ElementHandle at a point in time. Can go stale if DOM changes.
- `locator` â€” returns a Locator that re-queries each time you interact with it. Preferred for reliability.
...
# Type (simulates keystroke-by-keystroke input â€” slower, more realistic)
