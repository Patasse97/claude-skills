...
"""
data_profiler.py â€” Full dataset profile with Data Quality Score (DQS).
...
def compute_dqs(profiles: list[dict], total_rows: int) -> dict:
    ...
    # Uniqueness (15%) â€” placeholder; duplicate detection needs full row comparison
    uniqueness = 90.0  # conservative default without row-level dedup check

    # Timeliness (10%) â€” placeholder; requires timestamp columns
...
def dqs_label(score: float) -> str:
    if score >= 85:
        return "PASS â€” Production-ready"
    elif score >= 65:
        return "WARN â€” Usable with documented caveats"
    else:
        return "FAIL â€” Remediation required before use"
...
def print_report(headers: list[str], profiles: list[dict], dqs: dict, total_rows: int, monitor: bool):
    ...
    score = dqs["score"]
    indicator = "ðŸŸ¢" if score >= 85 else ("ðŸŸ¡" if score >= 65 else "ðŸ”´")
    ...
    for dim, val in dims.items():
        bar = int(val / 5)
        print(f"  {dim.capitalize():<14} {val:>5.1f}  {'â–ˆ' * bar}{'â–‘' * (20 - bar)}")
    ...
    for p in profiles:
        ...
        if p["null_pct"] > 30:
            status = "ðŸ”´"
            col_issues.append(f"{p['null_pct']}% nulls â€” investigate root cause")
        elif p["null_pct"] > 10:
            status = "ðŸŸ¡"
            col_issues.append(f"{p['null_pct']}% nulls â€” impute cautiously")
        elif p["null_pct"] > 1:
            col_issues.append(f"{p['null_pct']}% nulls â€” impute with indicator")
        if p["is_constant"]:
            status = "ðŸŸ¡"
            col_issues.append("Constant column â€” zero variance, likely useless")
        if p["is_high_cardinality"] and p["inferred_type"] == "string":
            col_issues.append("High-cardinality string â€” check if categorical or free-text")
