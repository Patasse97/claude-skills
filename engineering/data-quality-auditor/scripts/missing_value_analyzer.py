#!/usr/bin/env python3
"""
missing_value_analyzer.py â€” Classify missingness patterns and recommend imputation strategies.
...
def classify_mechanism(col: str, mask: list[bool], all_masks: dict[str, list[bool]]) -> str:
    ...
    # Check if nulls are clustered (time/positional pattern) â€” proxy for MNAR
    ...
    if correlated_cols:
        return f"MAR (likely) â€” co-occurs with nulls in: {', '.join(correlated_cols[:3])}"
    elif clustered:
        return "MNAR (possible) â€” nulls are spatially clustered, may reflect a systematic gap"
    else:
        return "MCAR (likely) â€” nulls appear random, no strong correlation detected"
...
def recommend_strategy(pct: float, col_type: str) -> str:
    ...
    if pct < 1:
        return "Drop rows â€” impact is negligible"
    if pct < 10:
        strategies = {
    ...
    return "Do NOT impute blindly â€” > 30% missing. Escalate to domain owner or consider dropping column"
...
def print_report(headers: list[str], rows: list[dict], masks: dict, threshold: float):
    ...
    if not results:
        ...
        for r in sorted(results, key=lambda x: -x["null_pct"]):
            indicator = "ðŸ”´" if r["null_pct"] > 30 else ("ðŸŸ¡" if r["null_pct"] > 10 else "ðŸŸ¢")
    ...
    if cooccur:
        ...
        print("-" * 64)
        for pair in cooccur:
            print(f"  {pair['col_a']} + {pair['col_b']}  â†’  {pair['co_null_rows']} rows both null")
