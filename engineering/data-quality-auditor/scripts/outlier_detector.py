...
"""
outlier_detector.py â€” Multi-method outlier detection for numeric columns.

Methods:
  iqr     â€” Interquartile Range (robust, non-parametric, default)
  zscore  â€” Standard Z-score (assumes normal distribution)
  mzscore â€” Modified Z-score via Median Absolute Deviation (robust to skew)
...
def detect_zscore(nums: list[float], threshold: float = 3.0) -> dict:
    ...
    if sigma == 0:
        return {"method": "Z-score", "outlier_count": 0, "outlier_pct": 0.0,
                "note": "Zero variance â€” all values identical"}
...
def detect_modified_zscore(nums: list[float], threshold: float = 3.5) -> dict:
    ...
    if mad == 0:
        return {"method": "Modified Z-score (MAD)", "outlier_count": 0, "outlier_pct": 0.0,
                "note": "MAD is zero â€” consider Z-score instead"}
...
def classify_outlier_risk(pct: float, col: str) -> str:
    ...
    if pct > 10:
        return "High outlier rate â€” likely systematic data quality issue or wrong data type"
    if pct > 5:
        return "Elevated outlier rate â€” investigate source; may be mixed populations"
    if pct > 1:
        return "Moderate â€” review individually; could be legitimate extremes or entry errors"
    if pct > 0:
        return "Low â€” verify extreme values against source; likely legitimate but worth checking"
    return "Clean â€” no outliers detected"


def analyze_column(col: str, nums: list[float], method: str, threshold: float) -> dict:
    if len(nums) < 4:
        return {"column": col, "status": "Skipped â€” fewer than 4 numeric values"}
...
def print_report(results: list[dict]):
    ...
    if flagged:
        ...
        for r in sorted(flagged, key=lambda x: -x.get("outlier_pct", 0)):
            ...
            indicator = "ðŸ”´" if pct > 5 else "ðŸŸ¡"
            ...
            if "mean" in r:
                print(f"     Mean: {r['mean']}  |  Std: {r['std']}  |  Threshold: Â±{r['threshold']}Ïƒ")
            if "median" in r:
