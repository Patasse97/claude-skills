...
Reference material for demo video scene design â€” colors, typography, animation timing, voice options, and pacing.
...
Background: dark with subtle purple-blue glow gradients. Screenshots: always `border-radius: 12px` with `box-shadow`. Easing: always `cubic-bezier(0.16, 1, 0.3, 1)` â€” never `ease` or `linear`.
