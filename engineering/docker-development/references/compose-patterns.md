...
### Key Patterns
- **Healthchecks on every service** â€” enables depends_on with condition
- **Named volumes** â€” data persists across container recreation
- **Explicit networks** â€” backend is internal (no external access)
- **env_file** â€” secrets not in compose file
- **Resource limits** â€” prevent runaway containers

---

## Development Override Pattern

### docker-compose.yml (base â€” production-like)
```yaml
services:
  app:
    build: .
...
### docker-compose.override.yml (dev â€” auto-loaded)
```yaml
services:
  app:
    build:
...
- Default Docker logging has NO size limit â€” production servers can run out of disk
