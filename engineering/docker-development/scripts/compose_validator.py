...
def validate_compose(parsed, strict=False):
    ...
    if version:
        findings.append({
            ...
            "message": f"'version: {version}' is deprecated in Compose V2 â€” remove it",
            "service": "(top-level)",
        })
    ...
    for name, svc in services.items():
        ...
        if "healthcheck" not in svc:
            findings.append({
                ...
                "message": f"No healthcheck defined â€” orchestrator can't detect unhealthy state",
                "service": name,
            })
        ...
        if image:
            if ":latest" in image:
                findings.append({
                    ...
                    "message": f"Using :latest tag on '{image}' â€” pin to specific version",
                    "service": name,
                })
                ...
                findings.append({
                    "severity": "high",
                    "category": "reproducibility",
                    "message": f"No tag on image '{image}' â€” defaults to :latest",
                    "service": name,
                })
        ...
        if isinstance(depends, list) and depends:
            findings.append({
                "severity": "medium",
                "category": "reliability",
                "message": "depends_on without condition: service_healthy â€” race condition risk",
                "service": name,
            })
                ...
                if v_str.startswith("./") or v_str.startswith("/"):
                    if "/var/run/docker.sock" in v_str:
                        findings.append({
                            "severity": "critical",
                            "category": "security",
                            "message": "Docker socket mounted â€” container has host Docker access",
                            "service": name,
                        })
        ...
        if "restart" not in svc and "build" not in svc:
            findings.append({
                "severity": "low",
                "category": "reliability",
                "message": "No restart policy â€” container won't auto-restart on failure",
                "service": name,
            })
        ...
        if "mem_limit" not in svc and "deploy" not in svc:
            findings.append({
                ...
                "category": "resources",
                "message": "No memory limit â€” container can consume all host memory",
                "service": name,
            })
    ...
    if "networks" not in parsed or not parsed["networks"]:
        if len(services) > 1:
            findings.append({
                "severity": "low",
                "category": "networking",
                "message": "No explicit networks â€” all services share default bridge network",
                "service": "(top-level)",
            })
...
def generate_report(content, output_format="text", strict=False):
    ...
    for f in findings:
        ...
        print(f"\n  {icon} {f['severity'].upper()} [{f['category']}] â€” {f['service']}")
        print(f"  {f['message']}")
...
def main():
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Strict mode â€” elevate warnings to higher severity",
    )
