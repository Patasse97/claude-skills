...
if [ -n "${AUTH_JWT_SECRET:-}" ] && [ ${#AUTH_JWT_SECRET} -lt 32 ]; then
  WARNINGS+=("AUTH_JWT_SECRET is shorter than 32 chars â€” insecure")
fi
...
```bash
#!/bin/bash
# scripts/scan-history.sh â€” scan entire git history for leaked secrets
...
### Step 2 â€” Identify Exposure Window
```bash
...
### Step 3 â€” Rotate Credential
Per service â€” rotate immediately:
- **AWS**: IAM console â†’ delete access key â†’ create new â†’ update everywhere
- **Stripe**: Dashboard â†’ Developers â†’ API keys â†’ Roll key
- **GitHub PAT**: Settings â†’ Developer Settings â†’ Personal access tokens â†’ Revoke â†’ Create new
- **DB password**: `ALTER USER app_user PASSWORD 'new-strong-password-here';`
- **JWT secret**: Rotate key (all existing sessions invalidated â€” users re-login)

### Step 4 â€” Update All Environments
```bash
...
### Step 5 â€” Remove from Git History
```bash
# WARNING: rewrites history â€” coordinate with team first
