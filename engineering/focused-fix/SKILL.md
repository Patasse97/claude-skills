---
name: "focused-fix"
description: "Use when the user asks to fix, debug, or make a specific feature/module/area work end-to-end. Triggers: 'make X work', 'fix the Y feature', 'the Z module is broken', 'focus on [area]'. Not for quick single-bug fixes â€” this is for systematic deep-dive repair across all files and dependencies."
---

# Focused Fix â€” Deep-Dive Feature Repair
...
This is NOT for quick single-bug fixes (use systematic-debugging for that). This is for when an entire feature or module needs systematic repair â€” tracing every dependency, reading logs, checking tests, mapping the full dependency graph.
...
## Protocol â€” STRICTLY follow these 5 phases IN ORDER
...
3. Map EVERY file in that folder â€” read each one, understand its purpose
...
### Phase 2: TRACE â€” Map All Dependencies (Inside AND Outside)
...
DEPENDENCY MAP:
  Inbound (this feature depends on):
    src/lib/db.ts â†’ used in auth/repository.ts (getUserById, createUser)
    src/lib/jwt.ts â†’ used in auth/service.ts (signToken, verifyToken)
    @prisma/client â†’ used in auth/repository.ts
    process.env.JWT_SECRET â†’ used in auth/service.ts
    process.env.DATABASE_URL â†’ used via prisma

  Outbound (depends on this feature):
    src/app/api/login/route.ts â†’ imports { login } from auth/service
    src/app/api/register/route.ts â†’ imports { register } from auth/service
    src/middleware.ts â†’ imports { verifyToken } from auth/service
...
- [ ] Check test coverage â€” are there untested code paths?
...
- Trace the data/control flow backward to verify â€” don't trust surface-level symptoms
...
DIAGNOSIS REPORT:
  ...
  CRITICAL:
    1. [HIGH] [file:line] â€” description of issue. Root cause: [confirmed explanation]
    2. [HIGH] [file:line] â€” description of issue. Root cause: [confirmed explanation]

  WARNINGS:
    1. [MED] [file:line] â€” description of issue
    2. [LOW] [file:line] â€” description of issue
...
### Phase 4: FIX â€” Repair Everything Systematically

Fix issues in this EXACT order:

1. **DEPENDENCIES FIRST** â€” fix broken imports, missing packages, wrong versions
2. **TYPES SECOND** â€” fix type mismatches at feature boundaries
3. **LOGIC THIRD** â€” fix actual business logic bugs
4. **TESTS FOURTH** â€” fix or create tests for each fix
5. **INTEGRATION LAST** â€” verify the feature works end-to-end with its consumers
...
**ESCALATION RULE â€” 3-Strike Architecture Check:**
...
1. Run ALL tests in the feature folder â€” every single one must pass
2. Run ALL tests in files that IMPORT from this feature â€” must pass
3. Run the full test suite if available â€” check for regressions
...
FOCUSED FIX COMPLETE:
  ...
  Changes:
    1. auth/service.ts â€” fixed token signing argument order
    2. auth/repository.ts â€” added null check for user lookup
    3. auth/middleware.ts â€” fixed async error handling
    4. auth/types.ts â€” aligned UserResponse type with actual DB schema

  Consumers verified:
    - src/app/api/login/route.ts âœ…
    - src/app/api/register/route.ts âœ…
    - src/middleware.ts âœ…
```

## Red Flags â€” STOP and Return to Current Phase

If you catch yourself thinking any of these, you are skipping phases:

- "I can see the bug, let me just fix it" â†’ STOP. You haven't traced dependencies yet.
- "Scoping is overkill, it's obviously just this file" â†’ STOP. That's always wrong for feature-level fixes.
- "I'll map dependencies after I fix the obvious stuff" â†’ STOP. You'll miss root causes.
- "The user said fix X, so I only need to look at X" â†’ STOP. Features have dependencies.
- "Tests are passing so I'm done" â†’ STOP. Did you run consumer tests too?
- "I don't need to check env vars for this" â†’ STOP. Config issues masquerade as code bugs.
- "One more fix should do it" (after 2+ cascading failures) â†’ STOP. Escalate.
- "I'll skip the diagnosis report, the fixes are obvious" â†’ STOP. Write it down.
...
| Excuse | Reality |
|---|---|
| "The feature is small, I don't need all 5 phases" | Small features have dependencies too. Phases 1-2 take minutes for small features â€” do them. |
...
- **`superpowers:systematic-debugging`** â€” Use within Phase 3 for root-cause tracing of individual complex bugs
- **`superpowers:verification-before-completion`** â€” Use within Phase 5 before claiming the feature is fixed
- **`scope`** â€” If you need to understand blast radius before starting, run scope first then focused-fix
...
| FIX | Fix in order: deps â†’ types â†’ logic â†’ tests â†’ integration | Fix log per issue |
