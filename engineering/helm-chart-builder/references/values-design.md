...
```yaml
# Bad â€” what does this mean?
replicaCount: 1
maxSurge: 25%

# Good â€” clear purpose, type, and constraints
...
```yaml
# Bad â€” broken without override
image:
  repository: ""           # Fails: no image
  tag: ""                  # Fails: no tag

# Good â€” works out of the box
image:
...
```yaml
# Bad â€” 5 levels deep, painful to override
container:
  spec:
    security:
      context:
...
# Good â€” 2 levels, easy to override with --set
securityContext:
  runAsNonRoot: true
```
...
```yaml
# Good â€” grouped by Kubernetes resource
service:
  type: ClusterIP
  port: 80
...
```yaml
# BAD â€” secret visible in chart package, git history, Helm release
database:
  password: "mysecretpassword"
  apiKey: "sk-abc123"

# GOOD â€” empty defaults with documentation
database:
...
```yaml
# BAD â€” won't work on any other cluster
ingress:
  host: app.my-company.internal
storageClass: gp3
registry: 123456789.dkr.ecr.us-east-1.amazonaws.com

# GOOD â€” generic defaults
ingress:
  host: chart-example.local
...
```yaml
# BAD â€” unclear, verb-based
createServiceAccount: true
doAutoScale: false
skipTLS: true

# GOOD â€” adjective-based, consistent
serviceAccount:
...
```yaml
# BAD â€” what are these? What types? What are valid options?
foo: bar
maxRetries: 3
mode: advanced
workers: 4

# GOOD â€” purpose, type, and constraints are clear
...
```yaml
# BAD â€” ambiguous: is empty string intentional?
annotations: ""
nodeSelector: ""

# GOOD â€” null/empty map means "not set"
annotations: {}
nodeSelector: {}
...
```yaml
# Production overrides only â€” don't repeat defaults
replicaCount: 3
...
- **Fails fast** â€” `helm install` rejects invalid values before rendering templates
- **Documents types** â€” self-documenting valid options (enums, ranges)
- **IDE support** â€” editors can autocomplete and validate values files
- **CI safety** â€” catches typos in value overrides early
