...
Validate values.yaml files against Helm best practices â€” documentation coverage,
    ...
    for entry in entries:
        ...
        if SNAKE_CASE_PATTERN.match(key):
            findings.append({
                "severity": "medium",
                "category": "naming",
                "message": f"Key '{entry['full_path']}' uses snake_case â€” Helm convention is camelCase",
            ...
            findings.append({
                "severity": "medium",
                "category": "naming",
                "message": f"Key '{entry['full_path']}' starts with uppercase â€” use camelCase",
            ...
            findings.append({
                "severity": "medium",
                "category": "naming",
                "message": f"Key '{entry['full_path']}' uses kebab-case â€” Helm convention is camelCase",
                "fix": f"Rename to camelCase: {to_camel_case(key)}",
                "line": entry["line"],
            })
    ...
    if total > 0:
        ...
        if coverage < 50:
            ...
            findings.append({
                "severity": "medium",
                "category": "documentation",
                "message": f"{coverage:.0f}% documentation coverage ({documented}/{total}) â€” aim for 80%+",
    ...
    for entry in entries:
        ...
        if entry["key"] == "tag" and entry["value"] in ("latest", '"latest"', "'latest'"):
            findings.append({
                "severity": "high",
                "category": "defaults",
                "message": f"image.tag defaults to 'latest' â€” not reproducible",
        ...
        if entry["key"] == "pullPolicy" and entry["value"] in ("Always", '"Always"', "'Always'"):
            findings.append({
                "severity": "low",
                "category": "defaults",
                "message": "imagePullPolicy defaults to 'Always' â€” 'IfNotPresent' is better for production",
        ...
        if entry["key"] == "resources" and entry["value_type"] == "empty_map":
            findings.append({
                "severity": "medium",
                "category": "defaults",
                "message": "resources defaults to {} â€” no requests or limits set",
    ...
    if max_depth > 4:
        ...
        for entry in deep_entries[:3]:  # Report first 3
            findings.append({
                ...
                "fix": "Flatten structure â€” max 3-4 levels deep for usability",
                "line": entry["line"],
            })
