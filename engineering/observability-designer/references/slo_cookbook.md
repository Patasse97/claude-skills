...
âŒ **Don't use**: CPU usage, memory usage, disk space as primary SLIs
- These are symptoms, not user-facing impacts

âŒ **Don't use**: Counts instead of rates or proportions
- "Number of errors" vs "Error rate"

âŒ **Don't use**: Internal metrics that users don't care about
...
```
Error Budget = (1 - 0.999) Ã— 30 days = 0.001 Ã— 30 days = 43.8 minutes
```
...
```
SLO_service â‰¤ min(SLO_inherent, âˆSLO_dependencies)
```

If your service depends on 3 other services each with 99.9% SLO:
```
Maximum_SLO = 0.999Â³ = 0.997 = 99.7%
```
