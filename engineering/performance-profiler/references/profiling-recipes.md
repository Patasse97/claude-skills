...
curl -X POST http://localhost:3000/debug/heap-snapshot
# Open both snapshots in Chrome â†’ Memory â†’ Compare
```
...
if (afterGC.heapUsed > baseline.heapUsed * 1.1) {
  console.warn('âš ï¸  Possible memory leak detected (>10% growth after GC)')
}
```
...
# Open flamegraph.svg in browser â€” look for wide bars = hot code paths
```
...
# In pprof UI: "Flame Graph" view â†’ find the tallest bars
```
...
pnpm build 2>&1 | grep -E "^\s+(Î»|â—‹|â—)" | sort -k4 -rh | head -20

# Check if a specific package is too large
# Visit: https://bundlephobia.com/package/moment@2.29.4
# moment: 67.9kB gzipped â†’ replace with date-fns (13.8kB) or dayjs (6.9kB)
...
-- Look for:
-- Seq Scan on large table â†’ needs index
-- Nested Loop with high rows â†’ N+1, consider JOIN or batch
-- Sort â†’ can index handle the sort?
-- Hash Join â†’ fine for moderate sizes
```
