...
class RetrievalEvaluator:
    ...
    def _generate_summary(self, metrics: Dict[str, float], num_queries: int) -> str:
        ...
        summary += f"\nPerformance Assessment:\n"
        if p1 >= 0.7:
            summary += "âœ“ Excellent precision - most queries return relevant results first\n"
        elif p1 >= 0.5:
            summary += "â—‹ Good precision - many queries return relevant results first\n"
        else:
            summary += "âœ— Poor precision - few queries return relevant results first\n"
        
        if r5 >= 0.8:
            summary += "âœ“ Excellent recall - finding most relevant documents\n"
        elif r5 >= 0.6:
            summary += "â—‹ Good recall - finding many relevant documents\n"
        else:
            summary += "âœ— Poor recall - missing many relevant documents\n"
