...
def get_secret(secret_name, region="us-east-1"):
    ...
    try:
        ...
        if code == "ResourceNotFoundException":
            ...
            raise RuntimeError("KMS decryption failed â€” check key permissions")
        raise
...
```python
# rotation_lambda.py â€” skeleton for custom rotation
...
```python
# secret_client.py â€” cross-cloud abstraction
class SecretClient:
...
1. **Dual-write phase** â€” Write to both old and new store simultaneously
2. **Dual-read phase** â€” Read from new store, fallback to old
3. **Cut-over** â€” Read exclusively from new store
4. **Cleanup** â€” Remove secrets from old store after grace period
...
- Never manually copy secrets between stores â€” always automate
...
- **AWS:** `aws-secretsmanager-caching-python` â€” caches with configurable TTL
- **Azure:** Built-in HTTP caching in SDK, or use Azure App Configuration
- **GCP:** No official caching library â€” implement in-process cache with TTL
...
3. Never cache secrets to disk â€” in-memory only
