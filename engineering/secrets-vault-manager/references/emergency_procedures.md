...
|----------|-----------|---------------|---------|
| **P0 â€” Critical** | Production credentials exposed publicly | Immediate (15 min) | Database password in public GitHub repo |
| **P1 â€” High** | Internal credentials exposed beyond intended scope | 1 hour | API key in build logs accessible to wider org |
| **P2 â€” Medium** | Non-production credentials exposed | 4 hours | Staging DB password in internal wiki |
| **P3 â€” Low** | Expired or limited-scope credential exposed | 24 hours | Rotated API key found in old commit history |
...
7. **Rotate adjacent credentials**
   ...
   - If a Vault token leaked, check what policies it had â€” rotate everything accessible
...
```
SECURITY INCIDENT â€” SECRET EXPOSURE
Severity: P0/P1
...
Blast radius assessment: [PENDING / COMPLETE â€” no unauthorized access / COMPLETE â€” unauthorized access detected]
...
```
Sealed State:
  Vault process running â†’ YES
  API responding â†’ YES (503 Sealed)
  Serving secrets â†’ NO
  All active leases â†’ FROZEN (not revoked)
  Audit logging â†’ NO

Unsealed State:
  Vault process running â†’ YES
  API responding â†’ YES (200 OK)
  Serving secrets â†’ YES
  Active leases â†’ RESUMING
  Audit logging â†’ YES
```
...
# Seal each node individually â€” leader last
...
- Applications lose secret access â€” prepare for cascading failures
...
2. Check audit devices: `vault audit list` â€” confirm all enabled
...
1. **Infrastructure credentials** â€” Database root passwords, cloud IAM admin keys
2. **Service credentials** â€” Application database users, API keys
3. **Integration credentials** â€” Third-party API keys, webhook secrets
4. **Human credentials** â€” Force password reset, revoke SSO sessions
...
| Signal | Escalation |
|--------|-----------|
| Single secret exposed in internal log | P2 â€” Rotate secret, add log masking |
| Secret in public repository (no evidence of use) | P1 â€” Immediate rotation, history scrub |
| Secret in public repository (evidence of unauthorized use) | P0 â€” Full incident response, legal notification |
| Vault node compromised | P0 â€” Seal cluster, rotate all accessible secrets |
| Cloud KMS key compromised | P0 â€” Create new key, re-encrypt all secrets, rotate all credentials |
| Audit log gap detected | P1 â€” Investigate cause, assume worst case for gap period |
| Multiple failed auth attempts from unknown source | P2 â€” Block source, investigate, rotate targeted credentials |
