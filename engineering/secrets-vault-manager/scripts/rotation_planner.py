...
def load_inventory(path):
    ...
    for i, entry in enumerate(data):
        if not isinstance(entry, dict):
            print(f"WARNING: Skipping entry {i} â€” not an object", file=sys.stderr)
            continue
        ...
        if not last_rotated:
            print(f"WARNING: '{name}' has no last_rotated date â€” marking as overdue", file=sys.stderr)
            last_rotated_dt = None
        else:
            try:
                last_rotated_dt = datetime.strptime(last_rotated, "%Y-%m-%d")
            except ValueError:
                print(f"WARNING: '{name}' has invalid date '{last_rotated}' â€” marking as overdue", file=sys.stderr)
                last_rotated_dt = None
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Policies:
              30d   Aggressive â€” all secrets rotate within 30 days max
              60d   Standard â€” 60-day maximum rotation window
              90d   Relaxed â€” 90-day maximum rotation window
