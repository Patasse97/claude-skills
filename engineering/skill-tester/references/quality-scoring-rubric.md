...
### Dimension Weights (Default: 4 dimensions Ã— 25%)
...
**Important**: By default, the quality scorer uses 4 dimensions Ã— 25% weights for backward compatibility. To include Security assessment, use:
```bash
python quality_scorer.py <skill_path> --include-security
```

When Security is enabled, all dimensions are rebalanced to 20% each (5 dimensions Ã— 20% = 100%).
...
- **POWERFUL Tier**: Requires Security score â‰¥ 70
- **STANDARD Tier**: Requires Security score â‰¥ 50
