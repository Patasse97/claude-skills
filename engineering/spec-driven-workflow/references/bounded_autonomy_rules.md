...
| Bug found in existing code unrelated to spec | **STOP** â€” file a separate issue | Do not fix unrelated bugs in a spec-scoped implementation. |
...
Questions:
1. Can I write a test? Partially â€” I know WHAT to test but not HOW (email? push? in-app?). +20%
2. Multiple interpretations? Yes â€” notification channel is unclear. +20%
3. Contradicts itself? No. +0%
4. Assuming user behavior? Yes â€” I am assuming they want email. +15%
5. Undocumented external system? Maybe â€” depends on notification service. +15%
...
|---------|---------|------|
| "While I'm here..." | Refactoring a utility function unrelated to the spec | Medium â€” unreviewed changes |
| "This would be easy to add..." | Adding a search filter the spec does not mention | High â€” untested, unspecified |
| "Users will probably want..." | Building a feature based on assumption | High â€” may conflict with future specs |
| "This is obviously needed..." | Adding logging, metrics, or caching not in NFRs | Medium â€” may be overkill or wrong approach |
| "The spec forgot to mention..." | Building something the spec excluded | Critical â€” may be deliberately excluded |
...
2. **Escalate** immediately â€” do not implement without approval.
...
**Options considered:**
  A. Email only â€” Pros: simple, reliable. Cons: not real-time.
  B. Email + in-app notification â€” Pros: covers both async and real-time. Cons: more implementation effort.
  C. Configurable per user â€” Pros: maximum flexibility. Cons: requires preference UI (not in spec).
...
**Options considered:**
  A. Show generic "link invalid" error â€” Pros: secure (no info leak). Cons: confusing for deleted user.
  B. Show "account not found" error â€” Pros: clear. Cons: confirms account deletion to link holder.
**My recommendation:** A. Security over clarity â€” do not reveal account existence.
...
**Breaking:** Yes â€” existing clients will get 400 errors (missing required field)
**Options considered:**
  A. Make "role" required as spec says â€” Pros: matches spec. Cons: breaks mobile app v2.1.
  B. Make "role" optional with default "user" â€” Pros: backward compatible. Cons: deviates from spec.
  C. Version the API (v2) â€” Pros: clean separation. Cons: maintenance burden.
