...
A spec has 8 mandatory sections. If a section does not apply, write "N/A â€” [reason]" so reviewers know it was considered, not skipped.
...
## Section 3: Functional Requirements â€” RFC 2119
...
1. **Atomic** â€” One behavior per requirement. Not "The system MUST authenticate users and log them in."
2. **Testable** â€” You can write a test that proves it works or does not.
3. **Numbered** â€” Sequential FR-N format for traceability.
4. **Specific** â€” No ambiguous adjectives ("fast", "secure", "user-friendly").
...
## Section 5: Acceptance Criteria â€” Given/When/Then
...
```
Given [precondition â€” the world is in this state]
When  [action â€” the user or system does this]
Then  [outcome â€” this observable result occurs]
And   [additional outcome â€” and also this]
```
...
```markdown
- EC-1: Empty email field â†’ Return 400 with error "EMAIL_REQUIRED". Do not call auth service.
- EC-2: Email exceeds 255 characters â†’ Return 400 with error "EMAIL_TOO_LONG".
- EC-3: OAuth provider returns 503 â†’ Return 503 with "Service temporarily unavailable". Retry after 30s.
- EC-4: Two users register same email simultaneously â†’ First succeeds, second gets 409 Conflict.
- EC-5: User clicks reset link after password was already changed â†’ Show "Link already used."
```
...
- 500 for unexpected errors (keep it generic â€” do not leak internals)
...
Out of Scope prevents scope creep during implementation. When someone says "while you're in there, could you also..." â€” point them to this section.

### Format

```markdown
- OS-1: Multi-factor authentication â€” Planned for Q3 (SPEC-045).
- OS-2: Social login beyond Google/GitHub â€” Insufficient user demand (< 2% requests).
- OS-3: Admin impersonation â€” Security review pending. Separate spec required.
- OS-4: Password strength meter UI â€” Nice-to-have, deferred to design sprint 12.
```
...
- EC-1: External API returns 5xx â†’ Log error, return cached data if < 1h old, else 503.
- EC-2: External API response schema changes â†’ Log warning, reject unmappable fields.
```
...
- EC-1: Migration fails mid-way â†’ Automatic rollback, alert ops team.
- EC-2: New schema has stricter constraints â†’ Log invalid rows, quarantine for manual review.
```
...
- EC-1: User submits reset for non-existent email â†’ Same success message (FR-5).
- EC-2: User clicks reset link twice â†’ Second click shows "already used" if password was changed.
- EC-3: Email delivery fails â†’ Log error, do not retry automatically.
- EC-4: User requests reset while already logged in â†’ Allow it, do not force logout.
