...
| `actual rows` | Real row count â€” compare against estimate |
...
**Red flags:**
- `Seq Scan` on a large table with a WHERE clause â€” missing index
- `actual rows` >> `rows` (estimated) â€” stale statistics, run `ANALYZE`
- `Nested Loop` with high loop count â€” consider hash join or add index
- `Sort` with `external merge` â€” not enough `work_mem`, spilling to disk
- `Buffers: shared read` much higher than `shared hit` â€” cold cache or table too large for memory

### MySQL â€” EXPLAIN FORMAT=JSON
...
**Key fields:**
- `query_block.select_id` â€” identifies subqueries
- `table.access_type` â€” `ALL` (full scan), `ref` (index lookup), `range`, `index`, `const`
- `table.rows_examined_per_scan` â€” how many rows the engine reads
- `table.using_index` â€” covering index (no table lookup needed)
- `table.attached_condition` â€” the WHERE filter applied
...
**Modes:**
- **Session** â€” connection assigned for entire client session (safest, least efficient)
- **Transaction** â€” connection returned to pool after each transaction (recommended)
- **Statement** â€” connection returned after each statement (cannot use transactions)
