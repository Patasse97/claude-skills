...
### INNER JOIN â€” matching rows in both tables
```sql
...
### Self JOIN â€” comparing rows within the same table
```sql
...
### CROSS JOIN â€” every combination (cartesian product)
```sql
...
### LATERAL JOIN (PostgreSQL) â€” correlated subquery as a table
```sql
...
### Multiple CTEs â€” chaining transformations
```sql
...
### Recursive CTE â€” hierarchical data
```sql
-- Organization tree
...
### Recursive CTE â€” path traversal
```sql
-- Category breadcrumb
...
### ROW_NUMBER â€” assign unique rank per partition
```sql
SELECT *, ROW_NUMBER() OVER (PARTITION BY department_id ORDER BY salary DESC) AS rank
FROM employees;
```

### RANK and DENSE_RANK â€” handle ties
```sql
...
### LAG / LEAD â€” access adjacent rows
```sql
SELECT date, revenue,
...
### EXISTS â€” correlated existence check
```sql
...
### NOT EXISTS â€” safer than NOT IN for NULLs
```sql
...
### Scalar subquery â€” single value
```sql
SELECT name, salary,
...
### Derived table â€” subquery in FROM
```sql
...
### GROUPING SETS â€” multiple grouping levels
```sql
...
### FILTER clause (PostgreSQL) â€” conditional aggregation
```sql
SELECT
  COUNT(*) AS total,
