#!/usr/bin/env python3
"""
confidence_interval.py â€” Confidence intervals for proportions and means.

Methods:
  proportion â€” Wilson score interval (recommended over normal approximation for small n or extreme p)
  mean       â€” t-based interval using normal approximation for large n
...
def mean_interval(n: int, mean: float, std: float, confidence: float) -> dict:
    ...
    if rel_moe and rel_moe > 20:
        precision_note = "Wide CI â€” consider increasing sample size for tighter estimates."
    elif rel_moe and rel_moe < 5:
        precision_note = "Tight CI â€” high precision estimate."
...
def print_report(result: dict):
    ...
    if result["type"] == "proportion":
        ...
        print(f"  Margin of error: Â±{result['margin_of_error']:.4%}")
        print()
        ...
        print(f"  Margin of error: Â±{result['margin_of_error']}")
        if result.get("relative_margin_of_error_pct") is not None:
            print(f"  Relative MoE: Â±{result['relative_margin_of_error_pct']:.1f}%")
        if result.get("precision_note"):
            print(f"\n  â„¹ï¸  {result['precision_note']}")
    ...
    print(f"  in this specific interval â€” it either is or it isn't.")
    print("=" * 60)
