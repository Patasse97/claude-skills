#!/usr/bin/env python3
"""
hypothesis_tester.py â€” Z-test (proportions), Welch's t-test (means), Chi-square (categorical).
...
def t_cdf(t: float, df: float) -> float:
    ...
    Uses the relation: P(T â‰¤ t) = I_{x}(df/2, 1/2) where x = df/(df+t^2).
...
def cramers_v(chi2: float, n: int, k: int) -> float:
    """CramÃ©r's V effect size for chi-square test."""
...
def ztest_proportions(cn: int, cx: int, tn: int, tx: int, alpha: float) -> dict:
    ...
    if se == 0:
        return {"error": "Standard error is zero â€” check input values."}
...
def ttest_means(cm: float, cs: float, cn: int, tm: float, ts: float, tn: int, alpha: float) -> dict:
    ...
    if se == 0:
        return {"error": "Standard error is zero â€” check std values."}
...
def chi2_test(observed: list[float], expected: list[float], alpha: float) -> dict:
    ...
    if any(e < 5 for e in expected):
        return {"warning": "Some expected values < 5 â€” chi-square approximation may be unreliable.",
...
def verdict(result: dict) -> str:
    ...
    es_name = "Cohen's h" if "cohens_h" in es else ("Cohen's d" if "cohens_d" in es else "CramÃ©r's V")
    ...
    if sig:
        lines.append(f"  âœ… VERDICT: The difference is real (p={p:.4f} < Î±={alpha}).")
        if es_interp in ("negligible", "small"):
            lines.append("  âš ï¸  BUT: Effect is small â€” confirm practical significance before shipping.")
        else:
            lines.append("  Effect size is meaningful. Recommend shipping if no negative guardrails.")
    else:
        lines.append(f"  âŒ VERDICT: Insufficient evidence to conclude a difference exists (p={p:.4f} â‰¥ Î±={alpha}).")
