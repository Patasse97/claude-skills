...
"""
sample_size_calculator.py â€” Required sample size per variant for A/B experiments.
...
def sample_size_proportion(baseline: float, mde: float, alpha: float, power: float) -> int:
    ...
    Uses the standard formula:
        n = (z_Î±/2 + z_Î²)Â² Ã— (p1(1âˆ’p1) + p2(1âˆ’p2)) / (p1 âˆ’ p2)Â²
...
def sample_size_mean(baseline_mean: float, baseline_std: float, mde: float, alpha: float, power: float) -> int:
    ...
    Uses:
        n = 2 Ã— ÏƒÂ² Ã— (z_Î±/2 + z_Î²)Â² / Î´Â²

    where Î´ = mde Ã— baseline_mean (absolute effect).
    ...
    if delta == 0:
        raise ValueError("MDE Ã— baseline_mean = 0. Cannot size experiment with zero effect.")
...
def print_report(
    ...
    print(f"  Statistical power (1âˆ’Î²): {power:.0%}")
    ...
    # Risk interpretation
    if n < 100:
        print("  âš ï¸  Very small sample â€” results may be sensitive to outliers.")
    elif n > 1_000_000:
        print("  âš ï¸  Very large sample required â€” consider increasing MDE or accepting lower power.")
    else:
        print("  âœ… Sample size is achievable for most web/app products.")
...
def print_table(test: str, baseline: float, mde: float, alpha: float,
    ...
    print("=" * 70)
    print(f"  SAMPLE SIZE TRADEOFF TABLE  (Î±={alpha}, baseline={'proportion' if test == 'proportion' else 'mean'})")
    print("=" * 70)
