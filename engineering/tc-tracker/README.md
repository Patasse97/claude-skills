...
- `scripts/tc_init.py` â€” Initialize `docs/TC/` in a project (idempotent)
- `scripts/tc_create.py` â€” Create a new TC record with sequential ID
- `scripts/tc_update.py` â€” Update fields, status, files, handoff, with atomic writes
- `scripts/tc_status.py` â€” View a single TC or the full registry
- `scripts/tc_validator.py` â€” Validate a record or registry against schema + state machine
...
- `references/tc-schema.md` â€” JSON schema reference
- `references/lifecycle.md` â€” State machine and transitions
- `references/handoff-format.md` â€” Session handoff structure
