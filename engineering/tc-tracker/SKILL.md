...
```
docs/TC/
â”œâ”€â”€ tc_config.json          # Project settings
â”œâ”€â”€ tc_registry.json        # Master index + statistics
â”œâ”€â”€ records/
â”‚   â””â”€â”€ TC-001-04-05-26-user-auth/
â”‚       â””â”€â”€ tc_record.json  # Source of truth
â””â”€â”€ evidence/
    â””â”€â”€ TC-001/             # Log snippets, command output, screenshots
```
...
Creates `docs/TC/`, `docs/TC/records/`, `docs/TC/evidence/`, `tc_config.json`, and `tc_registry.json`. Idempotent â€” re-running reports "already initialized" with current stats.
...
- `progress_summary` â€” what has been done
- `next_steps` â€” ordered list of remaining actions
- `blockers` â€” anything preventing progress
- `key_context` â€” critical decisions, gotchas, patterns the next bot must know
- `files_in_progress` â€” files being edited and their state (`editing`, `needs_review`, `partially_done`, `ready`)
- `decisions_made` â€” architectural decisions with rationale and timestamp
...
1. **State machine** â€” only valid transitions are allowed.
2. **Sequential IDs** â€” `revision_history` uses `R1, R2, R3...`; `test_cases` uses `T1, T2, T3...`.
3. **Append-only history** â€” revision entries are never modified or deleted.
4. **Approval consistency** â€” `approved=true` requires `approved_by` and `approved_date`.
5. **TC ID format** â€” must match `TC-NNN-MM-DD-YY-slug`.
6. **Sub-TC ID format** â€” must match `TC-NNN.A` or `TC-NNN.A.N`.
7. **Atomic writes** â€” JSON is written to `.tmp` then renamed.
8. **Registry stats** â€” recomputed on every registry write.
...
- Surface questions only when genuinely needed ("This work doesn't match any active TC â€” create one?"), and ask once per session, not per file.
...
|--------------|--------------|-----------------|
| Editing `revision_history` to "fix" a typo | History is append-only â€” tampering destroys the audit trail | Add a new revision that corrects the field |
...
| Reusing TC IDs after deletion | Breaks the sequential guarantee and confuses history | Increment forward only â€” never recycle |
| Letting `next_steps` go stale | Defeats the purpose of handoff | Update on every milestone, even if it's "nothing changed" |

## Cross-References

- `engineering/changelog-generator` â€” Generates Keep-a-Changelog release notes from Conventional Commits. Pair it with TC tracker: TC for the granular per-change audit trail, changelog for user-facing release notes.
- `engineering/tech-debt-tracker` â€” For tracking long-lived debt items rather than discrete code changes.
- `engineering/focused-fix` â€” When a bug fix needs systematic feature-wide repair, run `/focused-fix` first then capture the result as a TC.
- `project-management/decision-log` â€” Architectural decisions made inside a TC's `decisions_made` block can also be promoted to a project-wide decision log.
- `engineering-team/code-reviewer` â€” Pre-merge review fits naturally into the `tested -> deployed` transition; capture the reviewer in `approval.approved_by`.

## References in This Skill

- [references/tc-schema.md](references/tc-schema.md) â€” Full JSON schema for TC records and the registry.
- [references/lifecycle.md](references/lifecycle.md) â€” State machine, valid transitions, and recovery flows.
- [references/handoff-format.md](references/handoff-format.md) â€” Session handoff structure and best practices.
