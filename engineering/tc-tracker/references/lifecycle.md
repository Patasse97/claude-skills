...
A TC moves through six implementation states. Transitions are validated on every write â€” invalid moves are rejected with a clear error.
...
1. Add a final revision: "Cancelled â€” reason: ...".
2. Move to `blocked`.
3. Add a `[CANCELLED]` tag.
4. Leave the record in place â€” never delete it (history is append-only).
...
| Long-lived `in_progress` TCs (weeks+) | Either too big â€” split into sub-TCs â€” or stalled and should be marked `blocked` |
