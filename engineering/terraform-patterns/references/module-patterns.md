...
- Pass values explicitly â€” no hidden data source lookups in children
...
### Workspace Caveats
- All environments share the same backend â€” less isolation than separate directories
...
### Why `for_each` > `count`
- `count` uses index â€” removing item 0 shifts all others, causing destroy/recreate
- `for_each` uses keys â€” removing a key only affects that resource
