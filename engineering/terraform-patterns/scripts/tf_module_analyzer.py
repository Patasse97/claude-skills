...
def check_naming(resources, data_sources):
    ...
    for r in resources:
        if not VALID_RESOURCE_NAME.match(r["name"]):
            issues.append({
                "severity": "medium",
                "message": f"Resource '{r['type']}.{r['name']}' uses non-standard naming â€” use lowercase with underscores",
            })
        if r["name"].startswith(r["provider"] + "_"):
            issues.append({
                "severity": "low",
                "message": f"Resource '{r['type']}.{r['name']}' name repeats the provider prefix â€” redundant",
            })
...
def check_variables(variables):
    ...
    for v in variables:
        if not v["has_description"]:
            issues.append({
                "severity": "medium",
                "message": f"Variable '{v['name']}' missing description â€” consumers won't know what to provide",
            })
        if not v["has_type"]:
            issues.append({
                "severity": "high",
                "message": f"Variable '{v['name']}' missing type constraint â€” accepts any value",
            })
    ...
    for expected, purpose in EXPECTED_FILES.items():
        if expected not in filenames:
            issues.append({
                ...
                "message": f"Missing '{expected}' â€” {purpose}",
            })
    return issues
...
def analyze_directory(tf_files):
    ...
    if not has_backend:
        findings.append({
            "severity": "high",
            "message": "No remote backend configured â€” state is stored locally",
        })
    ...
    if not has_tf_version:
        findings.append({
            "severity": "medium",
            "message": "No required_version constraint â€” any Terraform version can be used",
        })
    ...
    for filename, content in tf_files.items():
        if filename not in ("providers.tf", "versions.tf", "backend.tf"):
            if re.search(r'^provider\s+"', content, re.MULTILINE):
                findings.append({
                    "severity": "medium",
                    "message": f"Provider configuration found in '{filename}' â€” keep providers in root module only",
                })
