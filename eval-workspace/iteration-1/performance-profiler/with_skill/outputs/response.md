...
Your symptoms (200ms â†’ 3s at 500 concurrent users) point to either database saturation, connection pool exhaustion, or N+1 queries. Here's the exact sequence to find out.
...
# Baseline load test â€” record P50/P95/P99 before touching anything
...
Run these three checks in parallel â€” each eliminates a different layer:

**2a. Database â€” is the query itself slow?**
...
**2b. N+1 detection â€” is Node making too many queries per request?**
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    if (queryCount > 3) {
      console.warn(`âš ï¸ ${req.method} ${req.url}: ${queryCount} queries`);
    }
  });
  next();
});
...
**2c. Connection pool â€” is it exhausted under load?**
...
This opens a flamegraph in your browser. Look for wide bars â€” those are where CPU time is spent. Common culprits: JSON serialization of massive result sets, synchronous operations blocking the event loop.
...
1. **Connection pool exhaustion** â€” default pool of 10 connections serving 500 users
2. **N+1 queries** â€” amplifies the pool problem (20 queries/request Ã— 500 users = 10,000 queries competing for 10 connections)
3. **Missing index** â€” Seq Scan gets worse as table grows
