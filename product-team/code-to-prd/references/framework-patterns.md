...
|--------|--------------|
| Routes | `react-router-dom` â€” `<Route path="...">` or `createBrowserRouter` |
...
| Aspect | Where to Look |
|--------|--------------|
| Routes | `app/` directory â€” `page.tsx` = route, folders = segments |
...
| Aspect | Where to Look |
|--------|--------------|
| Routes | `pages/` directory â€” filename = route |
...
| Aspect | Where to Look |
|--------|--------------|
| Routes | `vue-router` â€” `routes` array in `router/index.ts` |
...
| Modules | `*.module.ts` â€” `@Module({ controllers, providers, imports })` |
| Services | `*.service.ts` â€” injected via constructor, contains business logic |
| DTOs | `*.dto.ts` â€” `class-validator` decorators define validation rules |
| Entities | `*.entity.ts` â€” TypeORM `@Entity()` / Prisma schemas |
| Auth | `@UseGuards(AuthGuard)`, `@Roles('admin')`, Passport strategies |
| Middleware | `*.middleware.ts`, registered in module `configure()` |
| Pipes | `ValidationPipe`, `ParseIntPipe` â€” input transformation |
...
| Aspect | Where to Look |
|--------|--------------|
| Routes | `urls.py` â€” `urlpatterns = [path('...', view)]` |
| Views | `views.py` â€” function-based or class-based views (`APIView`, `ViewSet`) |
| Models | `models.py` â€” `class MyModel(models.Model)` with field definitions |
| Forms | `forms.py` â€” `ModelForm`, `Form` with validation |
| Serializers | `serializers.py` (DRF) â€” `ModelSerializer`, field-level validation |
| Admin | `admin.py` â€” `@admin.register`, `list_display`, `search_fields`, `list_filter` |
| Templates | `templates/` â€” Jinja2/Django template HTML files |
| Middleware | `MIDDLEWARE` in `settings.py` |
| Auth | `django.contrib.auth`, `rest_framework.permissions`, `@login_required` |
| Signals | `signals.py` â€” `post_save`, `pre_delete` hooks (hidden business logic) |
| Management commands | `management/commands/` â€” CLI operations |
| Celery tasks | `tasks.py` â€” async/background operations |
...
| ViewSets | `viewsets.py` â€” `ModelViewSet` (full CRUD), `ReadOnlyModelViewSet` |
| Serializers | `serializers.py` â€” field types, validators, nested relations |
...
| Models | Pydantic `BaseModel` classes â€” request/response schemas |
| Dependencies | `Depends(...)` â€” auth, DB sessions, shared logic |
