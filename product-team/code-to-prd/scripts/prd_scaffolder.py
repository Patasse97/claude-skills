...
def generate_readme(project_name: str, routes: List[Dict], summary: Dict, date: str) -> str:
    ...
    lines = [
        f"# {project_name} â€” Product Requirements Document",
        "",
        f"> Generated: {date}",
        "",
        "## System Overview",
    ...
    for i, route in enumerate(routes, 1):
        ...
        lines.append(f"| {i} | {name} | `{path}` | <!-- TODO --> | [â†’](./pages/{filename}) |")
    ...
    lines.extend([
        ...
        "<!-- TODO: Global rules â€” delete confirmations, default sort, etc. -->",
        "",
    ])
...
## Overview
<!-- TODO: 2-3 sentences â€” core function and use case -->

## Layout
<!-- TODO: Region breakdown â€” search area, table, detail panel, action bar, etc. -->
...
### Page Load
<!-- TODO: What happens on mount â€” default queries, preloaded data -->
...
def generate_page_relationships(routes: List[Dict]) -> str:
    ...
    if len(routes) > 20:
        lines.append(f"  â””â”€â”€ ... ({len(routes) - 20} more)")
...
def scaffold(analysis: Dict[str, Any], output_dir: Path, project_name: Optional[str] = None):
    ...
    print(f"\nâœ… PRD scaffold complete: {output_dir}")
...
def main():
    ...
    if args.dry_run:
        routes = analysis.get("routes", {}).get("pages", [])
        print(f"Dry run â€” would create in {output_dir}/:\n")
