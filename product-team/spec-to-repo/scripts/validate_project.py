#!/usr/bin/env python3
"""
validate_project.py â€” Validate a generated project directory for common issues.
...
def check_readme(root):
    ...
    if size < 50:
        return {"name": "readme", "status": "WARN", "message": f"README.md is only {size} bytes â€” likely incomplete"}
...
def check_env_example(root, all_files):
    ...
    if not uses_env:
        return {"name": "env_example", "status": "PASS", "message": "No env vars detected â€” .env.example not required"}
...
def check_no_env_file(root):
    ...
    if os.path.isfile(path):
        return {"name": "no_env_committed", "status": "FAIL", "message": ".env file found â€” secrets may be committed"}
