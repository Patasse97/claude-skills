...
```
TRIGGER â†’ [CONDITION(s)] â†’ ACTION(s)
```
...
Rule Logic:
  IF component = "Backend" â†’ assign to @backend-lead
  IF component = "Frontend" â†’ assign to @frontend-lead
  IF component = "DevOps" â†’ assign to @devops-lead
```

### 2. SLA Warning â€” Stale Issues

```yaml
Trigger: Scheduled (daily at 9am)
Condition: JQL = "status != Done AND updated <= -5d AND priority in (High, Highest)"
Action: 
  - Add comment: "âš ï¸ This {{issue.priority.name}} issue hasn't been updated in 5+ days."
...
### 5. Sub-Task Completion â†’ Parent Transition
...
Action:
  ...
  - Comment: "âš¡ Auto-escalated: High-priority bug open for 24+ hours."
...
Action:
  ...
  - Create Confluence page:
    Title: "Release Notes â€” {{version.name}}"
    Content: List of resolved issues with types and summaries
```

### 9. Workload Balancer â€” Round-Robin Assignment
...
Action:
  ...
  - Send Slack: "#blockers: ðŸš¨ {{issue.key}} marked as Blocker by {{initiator.displayName}}"
...
1. **Name rules descriptively** â€” "Auto-assign Backend bugs to @dev-lead" not "Rule 1"
2. **Add conditions before actions** â€” prevent unintended execution
3. **Use JQL conditions** for precision â€” field conditions can miss edge cases
4. **Test in a sandbox project first** â€” automation mistakes can be destructive
5. **Set rate limits** â€” avoid infinite loops (Rule A triggers Rule B triggers Rule A)
6. **Monitor rule execution** â€” check Automation audit log weekly
7. **Document business rules** â€” explain WHY the rule exists, not just WHAT it does
8. **Use branches (if/else)** over separate rules â€” reduces rule count, easier to maintain
9. **Disable before deleting** â€” observe for a week to ensure no side effects
10. **Version your automation** â€” export rules as JSON backup before major changes
