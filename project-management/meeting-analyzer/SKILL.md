...
description: Analyzes meeting transcripts and recordings to surface behavioral patterns, communication anti-patterns, and actionable coaching feedback. Use this skill whenever the user uploads or points to meeting transcripts (.txt, .md, .vtt, .srt, .docx), asks about their communication habits, wants feedback on how they run meetings, requests speaking ratio analysis, mentions filler words or conflict avoidance, or wants to compare their communication across time periods. Also trigger when users mention tools like Granola, Otter, Fireflies, or Zoom transcripts. Even if the user just says "look at my meetings" or "how do I come across in meetings" â€” use this skill.
---

# Meeting Insights Analyzer

> Originally contributed by [maximcoding](https://github.com/maximcoding) â€” enhanced and integrated by the claude-skills team.
...
- Identify speaker labels â€” look for patterns like `Speaker 1:`, `[John]:`, `John Smith  00:14:32`, VTT/SRT cue formatting
...
If timestamps are missing, degrade gracefully â€” skip timing-dependent metrics (speaking pace, pause analysis) but still run text-based analysis.

### 3. Analyze

Run all applicable analysis modules below. Each module is independent â€” skip any that don't apply (e.g., skip speaking ratios if there are no speaker labels).
...
- **Turn count** â€” how many times each person spoke
- **Average turn length** â€” words per uninterrupted speaking turn
- **Longest monologue** â€” flag turns exceeding 60 seconds or 200 words
- **Interruption detection** â€” a turn that starts within 2 seconds of the previous speaker's last timestamp, or mid-sentence breaks
...
- Topic changes after tension (speaker A raises problem â†’ user pivots to logistics)
...
- The full quote (with surrounding context â€” 2 turns before and after)
...
- Contextual spikes â€” do fillers increase in specific situations? (e.g., when responding to a senior stakeholder, when giving negative feedback, when asked a question cold)
...
- **Building** (extends another's idea): "That's interesting â€” what if we also Z?"
...
- **Decision clarity**: Were decisions explicitly stated? ("So we're going with option B â€” Sarah owns the follow-up by Friday.")
...
Flag energy drops â€” moments where the user's engagement visibly decreases (shorter turns, less substantive responses). These often correlate with discomfort, boredom, or avoidance.

---

### 4. Output the Report

Structure the final output as a single cohesive report. Use this skeleton â€” omit any section where data was insufficient:

```markdown
# Meeting Insights Report

**Period**: [earliest date] â€“ [latest date]
...
### Facilitation
[Only if applicable â€” agenda, decisions, action items]
...
[Only if prior analysis exists â€” delta on key metrics]
```
...
- Tracking setup â€” save current metrics as a baseline for future comparison
...
- **Zoom**: Settings â†’ Recording â†’ enable "Audio transcript". Download `.vtt` from cloud recordings.
...
- **Fireflies.ai**: Export as `.docx` or `.json` â€” both work.
...
|---|---|---|
| Analyzing without speaker labels | Per-person metrics impossible â€” results are generic word clouds | Ask user to re-export with speaker identification enabled |
| Running all modules on a 5-minute standup | Overkill â€” filler word and conflict analysis need 20+ min meetings | Auto-detect meeting length and skip irrelevant modules |
...
| Analyzing a single meeting in isolation | One meeting is a snapshot, not a pattern â€” conclusions are unreliable | Require 3+ meetings minimum for trend-based coaching |
...
| Skill | Relationship |
|-------|-------------|
| `project-management/senior-pm` | Broader PM scope â€” use for project planning, risk, stakeholders |
| `project-management/scrum-master` | Agile ceremonies â€” pairs with meeting-analyzer for retro quality |
| `project-management/confluence-expert` | Store meeting analysis outputs as Confluence pages |
| `c-level-advisor/executive-mentor` | Executive communication coaching â€” complementary perspective |
