...
- **Integration Complexity Index (ICI):** Number of integration points Ã— complexity factor
- **Performance Risk Factor (PRF):** Historical performance variance in similar projects

**Formula:** `Technical Risk Score = (TMS Ã— 0.3 + ICI Ã— 0.4 + PRF Ã— 0.3) Ã— 1.2`
...
**Formula:** `Resource Risk Score = (SAI Ã— 0.4 + TSF Ã— 0.3 + CUR Ã— 0.3) Ã— 1.1`
...
```
EMV = Î£(Probability Ã— Impact) for all financial risk scenarios

Cost Escalation Model:
- Labor cost inflation: Historical rate Â± standard deviation
...
4. **EMV Calculation:** Probability Ã— Financial Impact for each risk
...
EMV = (0.6 Ã— $50K) + (0.3 Ã— $200K) + (0.1 Ã— $500K)
...
```
Portfolio Variance = Î£(Individual Project Variance) + 2Î£(Correlation Ã— StdDev1 Ã— StdDev2)
...
```
Project Option Value = Sâ‚€ Ã— N(dâ‚) - K Ã— e^(-rÃ—T) Ã— N(dâ‚‚)

Where:
Sâ‚€ = Current project value estimate
...
```
If (Risk EMV > Mitigation Cost Ã— 1.5):
...
- Cost efficiency: Mitigation cost Ã· Risk EMV reduction
...
**Acceptance Criteria:**
- Low impact Ã— Low probability risks
...
Average Resolution Time = Î£(Risk Resolution Days) / Number of Resolved Risks
...
```
Risk-Adjusted NPV = Standard NPV Ã— Risk Adjustment Factor

Risk Adjustment Factor = 1 - (Project Risk Score Ã— Risk Penalty Rate)
