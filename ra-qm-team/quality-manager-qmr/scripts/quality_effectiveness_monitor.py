...
class QMSEffectivenessMonitor:
        ...
        if metric.unit == "PPM" or "rate" in metric.metric_name.lower():
            ...
            if historical_values:
                ...
                if avg_defect_rate > 0:
                    ...
                    # Simplified sigma conversion (actual uses 1.5Ïƒ shift)
                    sigma_map = {
    ...
    def generate_management_review_summary(self, report: QMSReport) -> str:
        ...
        if report.critical_alerts > 0:
            summary.append("ðŸ”´ CRITICAL ALERTS REQUIRING IMMEDIATE ATTENTION:")
            for alert in [a for a in report.predictive_alerts if a.get("risk_level") == "high"]:
                summary.append(f"  â€¢ {alert['metric']}: forecast {alert['forecast_value']} (from {alert['current_value']})")
            summary.append("")

        summary.append("ðŸ“ˆ TOP IMPROVEMENT OPPORTUNITIES:")
        ...
        summary.append("ðŸŽ¯ RECOMMENDED ACTIONS:")
...
def format_qms_report(report: QMSReport) -> str:
    ...
    if report.predictive_alerts:
        ...
        for alert in report.predictive_alerts[:5]:
            lines.append(f"  âš  {alert['metric']}: {alert['current_value']} â†’ {alert['forecast_value']} ({alert['risk_level']})")
