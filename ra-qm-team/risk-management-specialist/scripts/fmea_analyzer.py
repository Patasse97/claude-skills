...
def format_fmea_text(report: FMEAReport) -> str:
    ...
    lines = [
        ...
        f"Critical Severity (â‰¥8): {report.summary['risk_distribution']['critical_severity']}",
        f"High RPN (â‰¥100): {report.summary['risk_distribution']['high_rpn']}",
    ...
    for i, entry in enumerate(report.entries, 1):
        marker = "âš " if entry.criticality in ["CRITICAL", "HIGH"] else "â€¢"
        lines.extend([
            f"",
            ...
            f"  Cause: {entry.cause}",
            f"  S={entry.severity} Ã— O={entry.occurrence} Ã— D={entry.detection} = RPN {entry.rpn} [{entry.criticality}]",
        ...
        if entry.revised_rpn > 0:
            lines.append(f"  Revised: S={entry.revised_severity} Ã— O={entry.revised_occurrence} Ã— D={entry.revised_detection} = RPN {entry.revised_rpn}")
