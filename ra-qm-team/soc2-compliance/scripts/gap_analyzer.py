            ...
            issues.append(
                {
                    "check": "operating_consistency",
                    "severity": "critical",
                    "detail": f"Control status is '{ctrl.get('status', 'Not Started')}' â€” operating evidence needed",
                }
            )
...
def build_report(
    ...
    if coverage_pct >= 90 and critical_gaps == 0:
        ...
        readiness = "Near Ready â€” address gaps before audit"
    elif coverage_pct >= 50:
        readiness = "Significant work needed"
    else:
        readiness = "Not ready â€” major build-out required"
...
def main():
    parser = argparse.ArgumentParser(
        description="SOC 2 Gap Analyzer â€” identifies gaps between current controls and SOC 2 requirements."
    )
    parser.add_argument(
        "--controls",
        type=str,
