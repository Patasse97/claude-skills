...
GITHUB_BASE = "https://github.com/Patasse97/claude-skills/tree/main"
...
def rewrite_skill_internal_links(content, skill_rel_path):
    ...
    assets/template.md, README.md â€” these exist in the repo but not in docs/.
...
def rewrite_relative_links(content, source_rel_path):
    ...
    def resolve_link(match):
        ...
        # These resolve to agents/product/cs-bar.md â€” only keep if they're
        ...
        if (resolved.startswith("agents/") and resolved.count("/") == 1
            ...
            # This is a sibling agent doc link â€” rewrite to flat docs slug
...
def generate_skill_page(skill, domain_key):
    ...
    title = re.sub(r"\s*[-â€”]\s*(POWERFUL|Core|Advanced)\s*$", "", title, flags=re.IGNORECASE)
    ...
    seo_title = f"{title} â€” {seo_suffix}"
    ...
    if fm_desc:
        ...
        if len(clean) > 150:
            ...
            truncated = clean[:150].rsplit(" ", 1)[0].rstrip(".,;:â€”-")
        ...
        description = f"{title} â€” {seo_ctx}. Works with {desc_platforms}."
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/{skill["rel_path"]}/SKILL.md">Source</a></span>
</div>
...
"""
        ...
        index_content = f'''---
title: "{domain_name} Skills â€” Agent Skills & Codex Plugins"
description: "{skill_count} {domain_name.lower()} skills â€” {domain_seo_ctx}. Works with Claude Code, Codex CLI, Gemini CLI, and OpenClaw."
---
    ...
    if os.path.isdir(agents_dir):
        for domain_folder in sorted(os.listdir(agents_dir)):
            ...
            for agent_file in sorted(os.listdir(domain_path)):
                ...
                agent_seo_title = f"{title} â€” AI Coding Agent & Codex Skill"
                ...
                if agent_fm_desc:
                    ...
                    if len(agent_clean) > 150:
                        agent_clean = agent_clean[:150].rsplit(" ", 1)[0].rstrip(".,;:â€”-")
                    agent_desc = f"{agent_clean}. Agent-native orchestrator for Claude Code, Codex, Gemini CLI."
                else:
                    agent_desc = f"{title} â€” agent-native AI orchestrator for {domain_label}. Works with Claude Code, Codex CLI, Gemini CLI, and OpenClaw."
...
<span class="meta-badge">:material-github: <a href="https://github.com/Patasse97/claude-skills/tree/main/{rel}">Source</a></span>
</div>
...
"""
        ...
        idx = f'''---
title: "AI Coding Agents â€” Agent-Native Orchestrators & Codex Skills"
description: "{agent_count} agent-native orchestrators for Claude Code, Codex CLI, and Gemini CLI â€” multi-skill AI agents across engineering, product, marketing, and more."
---
        ...
        for cmd_file in sorted(os.listdir(commands_dir)):
            ...
            if cmd_fm_desc:
                ...
                if len(cmd_clean) > 150:
                    cmd_clean = cmd_clean[:150].rsplit(" ", 1)[0].rstrip(".,;:â€”-")
                cmd_desc = f"{cmd_clean}. Slash command for Claude Code, Codex CLI, Gemini CLI."
            else:
                cmd_desc = f"/{cmd_name} â€” slash command for Claude Code, Codex CLI, and Gemini CLI. Run directly in your AI coding agent."

            page = f'''---
title: "/{cmd_name} â€” Slash Command for AI Coding Agents"
...
"""
        ...
        idx = f'''---
title: "Slash Commands â€” AI Coding Agent Commands & Codex Shortcuts"
description: "{cmd_count} slash commands for Claude Code, Codex CLI, and Gemini CLI â€” sprint planning, tech debt analysis, PRDs, OKRs, and more."
---
