#!/usr/bin/env bash
# review-new-skills.sh â€” Run Tessl + internal auditors on new/changed skills
#
# Usage:
...
for skill_dir in "${SKILL_DIRS[@]}"; do
  if [ ! -f "$skill_dir/SKILL.md" ]; then
    echo "â­  $skill_dir â€” no SKILL.md, skipping"
    continue
  fi

  echo "â”â”â” $skill_dir â”â”â”"
...
" 2>/dev/null || echo "0")
    ...
    if [ "$TESSL_SCORE" -ge "$THRESHOLD" ]; then
      echo "  âœ… Tessl:    ${TESSL_SCORE}/100 (desc: ${TESSL_DESC}%, content: ${TESSL_CONTENT}%)"
    else
      echo "  âš ï¸  Tessl:    ${TESSL_SCORE}/100 (desc: ${TESSL_DESC}%, content: ${TESSL_CONTENT}%) â€” BELOW THRESHOLD"
    fi
  else
    echo "  â­  Tessl:    not installed (npm install -g tessl)"
  fi
...
" 2>/dev/null || echo "0/ERROR")
  echo "  ðŸ“ Structure: $STRUCT_SCORE"
...
" 2>/dev/null || echo "ERROR")
    echo "  ðŸ§ª Scripts:  $SCRIPT_RESULT"
  fi
...
try:
    ...
    print('ERROR')
" 2>/dev/null || echo "ERROR")
  echo "  ðŸ”’ Security: $SEC_RESULT"

  # Verdict
  if [ "$TESSL_SCORE" -ge "$THRESHOLD" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    RESULTS+=("âœ… $skill_dir: ${TESSL_SCORE}/100")
  else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    RESULTS+=("âš ï¸  $skill_dir: ${TESSL_SCORE}/100 â€” below ${THRESHOLD}")
  fi
